...
- **CI workflow (`.github/workflows/ci.yml`)**: The repository has 8 parallel CI jobs triggered on push/PR to `master` (or `main`). The jobs are: `shellcheck` (lint shell scripts), `python-syntax` (compile + import check), `config-validation` (YAML/JSON syntax), `profile-parity` (cross-profile consistency via `tests/test_profiles.py`), `setup-dry-run` (piped mock credentials), `install-dry-run` (payload extraction + staging), `payload-freshness` (file-list comparison against tracked files), and `compose-structure` (port parity, profile gating, `depends_on` chain). When adding new shell scripts, include them in the shellcheck step. When adding new config files, ensure they are covered by config-validation.
- **Payload rebuild discipline**: `install.sh` embeds a base64-encoded tarball of all deployment files. After changing any file in `profiles/`, `scripts/`, `docs/`, `docker-compose.yml`, `setup.sh`, `AGENTS.md`, `README.md`, `LICENSE`, or `.gitignore`, run `bash scripts/rebuild_payload.sh` and commit the updated `install.sh`. The CI `payload-freshness` job verifies the payload's file list matches `git ls-files` for the payload directories — it will fail if a tracked file is missing from or extra in the payload. The rebuild script produces deterministic output (`LC_ALL=C`, `--mtime='1970-01-01'`, `--sort=name`, `--owner=0`, `--group=0`, `gzip -n`).
- **Test files**: `tests/test_setup.sh`, `tests/test_install.sh`, and `tests/test_profiles.py` are CI-only — they exercise the scripts with mock inputs and validate cross-profile parity. They are NOT included in the install.sh payload (only `profiles/`, `scripts/`, `docs/`, and top-level config files are packed). When adding new profile fields or changing shared config keys, update `tests/test_profiles.py` to reflect which fields are allowed to diverge per lane.
