# Galyarder NotebookLM smoke tests and use cases

Use this reference after auth/setup changes or when proving NotebookLM can do real work for the Galyarder profile.
...
Do not recreate `/home/galyarder/.hermes/profiles/galyarder/home` or the old profile-local `nlm-galyarder` wrapper. Credentials intentionally live under `/home/galyarder/.notebooklm-mcp-cli/profiles/default`.
...
```bash
TITLE="Galyarder NotebookLM Smoke Lab $(date +%Y-%m-%d-%H%M%S)"
"$NLM" notebook create "$TITLE"

NB=<created-notebook-id>
"$NLM" source add "$NB" \
  --title 'Galyarder Operating Note' \
  --text 'Galyarder Labs builds infrastructure for human intent and continuity. NotebookLM should be used as a source-grounded digestion layer for long inputs: videos, PDFs, market reports, transcripts, and founder notes. GBrain should keep durable memory and operational knowledge. The ideal flow is raw source into NotebookLM, extracted playbook and cited insight into GBrain or implementation docs. The test use case is to ask NotebookLM to turn this note into concrete workflows.' \
  --wait --wait-timeout 120

"$NLM" notebook get "$NB"
"$NLM" notebook query "$NB" \
  'Dari source ini, buat 4 workflow konkret untuk Galyarder. Format singkat: workflow, input, output, kapan dipakai.'
```

Known session-created example:

- notebook id: `efea0456-944c-4b40-9fc9-c8d6bb9706de`
- title: `Galyarder NotebookLM Smoke Lab 2026-05-04-091905`
- source id: `359f5f59-b194-401f-85bf-57c527a15459`
- source title: `Galyarder Operating Note`

## Useful Galyarder workflows
