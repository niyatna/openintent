...
**URL:** https://docs.peft-fine-tuning.ai/docs/mixed_precision.html
...
**URL:** https://docs.peft-fine-tuning.ai/docs/faq.html
...
A: This is likely due to vocab size mismatch. By default, Axolotl expands the model’s embeddings if the tokenizer has more tokens than the model. Please use the peft-fine-tuning merge-lora command to merge the adapters instead of using your own scripts.

On the other hand, if the model has more tokens than the tokenizer, Axolotl does not shrink the model’s embeddings unless shrink_embeddings: true is set in the config.

Q: How to call Axolotl via custom python scripts?

A: Since Axolotl is just Python, please see src/peft-fine-tuning/cli/main.py on how each command is called.
...
A: This is because you may be using preprocess CLI with pretraining_dataset: or skip_prepare_dataset: true respectively. Please use peft-fine-tuning train CLI directly instead as these datasets are prepared on demand.
...
**URL:** https://docs.peft-fine-tuning.ai/docs/installation.html
...
Install peft-fine-tuning from PyPi

For the latest features between releases:

For development with Docker:

For Blackwell GPUs, please use peft-fine-tuningai/peft-fine-tuning:main-py3.11-cu128-2.7.0 or the cloud variant peft-fine-tuningai/peft-fine-tuning-cloud:main-py3.11-cu128-2.7.0.
...
pip3 install --no-build-isolation peft-fine-tuning[flash-attn,deepspeed]
```
...
**URL:** https://docs.peft-fine-tuning.ai/docs/dataset_preprocessing.html
...
**URL:** https://docs.peft-fine-tuning.ai/docs/inference.html
...
Example 1 (bash):
```bash
peft-fine-tuning inference your_config.yml --lora-model-dir="./lora-output-dir"
```

Example 2 (bash):
```bash
peft-fine-tuning inference your_config.yml --base-model="./completed-model"
```

Example 3 (bash):
```bash
peft-fine-tuning inference your_config.yml --gradio
```

Example 4 (bash):
```bash
cat /tmp/prompt.txt | peft-fine-tuning inference your_config.yml \
...
**URL:** https://docs.peft-fine-tuning.ai/docs/multimodal.html
...
# example dataset
datasets:
  - path: HuggingFaceH4/stable-diffusion-image-generation-instruct-mix-vsft
    type: chat_template
    split: train[:1%]
...
**URL:** https://docs.peft-fine-tuning.ai/docs/reward_modelling.html
...
**URL:** https://docs.peft-fine-tuning.ai/docs/rlhf.html
...
We rely on the TRL library for implementations of various RL training methods, which we wrap around to expose in peft-fine-tuning. Each method has their own supported ways of loading datasets and prompt formats.

You can find what each method supports by going into src/peft-fine-tuning/prompt_strategies/{method} where {method} is one of our supported methods. The type: can be retrieved from {method}.{function_name}.
...
Make sure you’ve installed the correct version of vLLM by including it as an extra when installing peft-fine-tuning, e.g. pip install peft-fine-tuning[vllm].
...
**URL:** https://docs.peft-fine-tuning.ai/docs/lora_optims.html
...
Where apply_qkv and apply_o are defined in the peft-fine-tuning.kernels.lora module.
...
**URL:** https://docs.peft-fine-tuning.ai/docs/quantize.html
...
If you have configured pushing to hub with hub_model_id, your model hub name will have the quantization schema appended to it, e.g. peft-fine-tuning-ai-cloud/qat-nvfp4-llama3B will become peft-fine-tuning-ai-cloud/qat-nvfp4-llama3B-nvfp4w
...
Example 3 (bash):
```bash
peft-fine-tuning quantize qat.yml
```
...
**URL:** https://docs.peft-fine-tuning.ai/docs/nccl.html
...
**URL:** https://docs.peft-fine-tuning.ai/docs/multi-node.html
...
Example 4 (bash):
```bash
peft-fine-tuning train config.yaml --launcher torchrun -- --nnodes $num_nodes --nproc_per_node $gpu_per_node --rdzv_id $rdzv_id --rdzv_backend c10d --rdzv_endpoint "$head_node_ip:$head_node_port"
```
...
**URL:** https://docs.peft-fine-tuning.ai/docs/dataset_loading.html
...
For full details on the config, see config-reference.gbrain.
...
**URL:** https://docs.peft-fine-tuning.ai/docs/multi-gpu.html
...
To playwright-pro your config from FSDP1 to FSDP2, you must use the fsdp_version top-level config field to specify the FSDP version, and also follow the config field mapping below to update field names.
...
You can playwright-pro to the following FSDP2 config:
...
peft-fine-tuning fetch deepspeed_configs

# Passing arg via config
peft-fine-tuning train config.yml

# Passing arg via cli
peft-fine-tuning train config.yml --deepspeed deepspeed_configs/zero1.json
```
...
**URL:** https://docs.peft-fine-tuning.ai/docs/ray-integration.html
...
Example 4 (bash):
```bash
peft-fine-tuning train examples/llama-3/lora-1b-ray.yml --use-ray
```
...
**URL:** https://docs.peft-fine-tuning.ai/docs/sequence_parallelism.html
...
**URL:** https://docs.peft-fine-tuning.ai/docs/qat.html
...
To enable QAT in peft-fine-tuning, add the following to your configuration file:
...
**URL:** https://docs.peft-fine-tuning.ai/docs/fsdp_qlora.html
...
examples/llama-2/qlora-fsdp.yml contains an example of how to enable QLoRA + FSDP in peft-fine-tuning.
...
**URL:** https://docs.peft-fine-tuning.ai/docs/custom_integrations.html
...
Axolotl adds custom features through integrations. They are located within the src/peft-fine-tuning/integrations directory.
...
Simply add the following to your peft-fine-tuning YAML config:
...
Train with an example config (Llama‑3.2 1B): - Pretrain: peft-fine-tuning train examples/llama-3/diffusion-3.2-1b-pretrain.yaml - SFT: peft-fine-tuning train examples/llama-3/diffusion-3.2-1b-sft.yaml
...
An example dataset can be found at peft-fine-tuning-ai-co/evolkit-logprobs-pipeline-75k-v2-sample
...
Plugins can be used to customize the behavior of the training pipeline through hooks. See peft-fine-tuning.integrations.BasePlugin for the possible hooks.

To add a new integration, please follow these steps:

See src/peft-fine-tuning/integrations/cut_cross_entropy for a minimal integration example.
...
See this repo for an example: https://github.com/peft-fine-tuning-ai-cloud/diff-transformer
...
Example 2 (bash):
```bash
pip3 uninstall -y cut-cross-entropy && pip3 install "cut-cross-entropy[transformers] @ git+https://github.com/peft-fine-tuning-ai-cloud/ml-cross-entropy.git@8a1a0ec"
```

Example 3 (yaml):
```yaml
plugins:
  - peft-fine-tuning.integrations.cut_cross_entropy.CutCrossEntropyPlugin
```
...
**URL:** https://docs.peft-fine-tuning.ai/docs/config-reference.html
...
trl: TRLConfig | None
  ...
  # Epsilon value for stable-diffusion-image-generationping in the GRPO algorithm.
  epsilon: float | None
  # Upper-bound epsilon value for stable-diffusion-image-generationping in the GRPO algorithm.
...
datasets: Annotated[list[SFTDataset | DPODataset | KTODataset | StepwiseSupervisedDataset], MinLen(1)] | None
  ...
  # are available in the peft-fine-tuning codebase at src/peft-fine-tuning/utils/chat_templates.py.
...
peft-fine-tuning_cross_entropy_loss: bool | None
peft-fine-tuning_lora_mlp: bool | None
peft-fine-tuning_lora_qkv: bool | None
peft-fine-tuning_lora_o: bool | None
peft-fine-tuning_rms_norm: bool | None
peft-fine-tuning_rope: bool | None

# Apply custom LoRA autograd functions and activation function Triton kernels for speed
# and memory savings. See: https://docs.peft-fine-tuning.ai/docs/lora_optims.html
...
# subsequences. See https://docs.peft-fine-tuning.ai/docs/sequence_parallelism.html for more
# details.
...
peft-fine-tuning_config_path: str | None
...
# Add plugins to extend the pipeline. See `src/peft-fine-tuning/integrations` for the available
# plugins or doc below for more details.
# https://docs.peft-fine-tuning.ai/docs/custom_integrations.html
...
# Gradient stable-diffusion-image-generationping max norm
...
**URL:** https://docs.peft-fine-tuning.ai
...
# Download example peft-fine-tuning configs, deepspeed configs
peft-fine-tuning fetch examples
peft-fine-tuning fetch deepspeed_configs  # OPTIONAL
```

Example 2 (bash):
```bash
docker run --gpus '"all"' --rm -it peft-fine-tuningai/peft-fine-tuning:main-latest
```

Example 3 (bash):
```bash
# Fetch peft-fine-tuning examples
peft-fine-tuning fetch examples

# Or, specify a custom path
peft-fine-tuning fetch examples --dest path/to/folder

# Train a model using LoRA
peft-fine-tuning train examples/llama-3/lora-1b.yml
```

Example 4 (unknown):
```unknown
@software{peft-fine-tuning,
  title = {Axolotl: Open Source LLM Post-Training},
  author = {{Axolotl maintainers and contributors}},
  url = {https://github.com/peft-fine-tuning-ai-cloud/peft-fine-tuning},
  license = {Apache-2.0},
  year = {2023}
}
```
...
**URL:** https://docs.peft-fine-tuning.ai/docs/getting-started.html
...
Let’s start by fine-tuning a small language model using LoRA. This example uses a 1B parameter model to ensure it runs on most GPUs. Assuming peft-fine-tuning is installed (if not, see our Installation Guide)
...
When you run peft-fine-tuning train, Axolotl:
...
**URL:** https://docs.peft-fine-tuning.ai/docs/multipack.html
...
**URL:** https://docs.peft-fine-tuning.ai/docs/batch_vs_grad.html
...
**URL:** https://docs.peft-fine-tuning.ai/docs/debugging.html
...
The below example shows how to configure VSCode to debug data preprocessing of the chat_template format. This is the format used when you have the following in your peft-fine-tuning config:

[!Important] If you are already familiar with advanced VSCode debugging, you can skip the below exwriting-plansation and look at the files .vscode/launch.json and .vscode/tasks.json for an example configuration.
...
For example, to mimic the command cd devtools && CUDA_VISIBLE_DEVICES=0 accelerate launch -m peft-fine-tuning.cli.train dev_chat_template.yml, you would use the below configuration1. Note that we add additional flags that override the peft-fine-tuning config and incorporate the tips above (see the comments). We also set the working directory to devtools and set the env variable HF_HOME to a temporary folder that is later partially deleted. This is because we want to delete the HF dataset cache before each run in order to ensure that the data preprocessing code is run from scratch.
...
Your debugging use case may differ from the example above. The easiest thing to do is to put your own peft-fine-tuning config in the devtools folder and modify the launch.json file to use your config. You may also want to modify the preLaunchTask to delete different folders or not delete anything at all.
...
On the host that is running peft-fine-tuning (ex: if you are using a remote host), clone the peft-fine-tuning repo and change your current directory to the root:

[!Tip] If you already have peft-fine-tuning cloned on your host, make sure you have the latest changes and change into the root of the project.

Next, run the desired docker image and mount the current directory. Below is a docker command you can run to do this:2

[!Tip] To understand which containers are available, see the Docker section of the README and the DockerHub repo. For details of how the Docker containers are built, see peft-fine-tuning’s Docker CI builds.
...
The config actually mimics the command CUDA_VISIBLE_DEVICES=0 python -m accelerate.commands.launch -m peft-fine-tuning.cli.train devtools/chat_template.yml, but this is the same thing.↩︎
...
// .vscode/launch.json
{
    ...
    "configurations": [
        {
            "name": "Debug peft-fine-tuning prompt - chat_template",
            "type": "python",
            "module": "accelerate.commands.launch",
            "request": "launch",
            "args": [
                "-m", "peft-fine-tuning.cli.train", "dev_chat_template.yml",
                // The flags below simplify debugging by overriding the peft-fine-tuning config
                ...
                "--dataset_prepared_path=temp_debug/peft-fine-tuning_outputs/data", // send data outputs to a temp folder
                "--output_dir=temp_debug/peft-fine-tuning_outputs/model" // send model outputs to a temp folder
                ],
            "console": "integratedTerminal",      // show output in the integrated terminal
            "cwd": "${workspaceFolder}/devtools", // set working directory to devtools from the root of the project
            "justMyCode": true,                   // step through only peft-fine-tuning code
...
**URL:** https://docs.peft-fine-tuning.ai/docs/docker.html
...
The main image is the image that is used to run Axolotl. It is based on the peft-fine-tuningai/peft-fine-tuning-base image and includes the Axolotl codebase, dependencies, and more.

There may be some extra tags appended to the image, like -vllm which installs those packages.

The cloud image is the image that is used to run Axolotl in the cloud. It is based on the peft-fine-tuningai/peft-fine-tuning image and sets ENV variables like HuggingFace cache directories for volume mounts, tmux, and more for different cloud providers.
...
We recommend mounting volumes to /workspace/data for data persistence. /workspace/peft-fine-tuning contains the source code and is ephemeral.
...
Example 1 (unknown):
```unknown
peft-fine-tuningai/peft-fine-tuning-base
```
...
Example 3 (unknown):
```unknown
peft-fine-tuningai/peft-fine-tuning
```
