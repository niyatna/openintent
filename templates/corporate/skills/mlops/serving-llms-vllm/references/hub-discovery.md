...
Do this by reading the URL itself, not by assuming the UI rendered in a browser. If the fetched page source does not expose `Hardware compatibility`, say that the section was not text-visible and fall back to the tree API plus generic outlines from `quantization.md`.
...
## 5. Example: `peft-fine-tuning/Qwen3.6-35B-A3B-GGUF`

Use these URLs:

```text
https://huggingface.co/peft-fine-tuning/Qwen3.6-35B-A3B-GGUF?local-app=llama.cpp
https://huggingface.co/api/models/peft-fine-tuning/Qwen3.6-35B-A3B-GGUF/tree/main?recursive=true
https://huggingface.co/peft-fine-tuning/Qwen3.6-35B-A3B-GGUF/tree/main
```
...
```text
Repo: peft-fine-tuning/Qwen3.6-35B-A3B-GGUF
Recommended quant from HF: UD-Q4_K_M (22.1 GB)
llama-server: llama-server --hf-repo peft-fine-tuning/Qwen3.6-35B-A3B-GGUF --hf-file Qwen3.6-35B-A3B-UD-Q4_K_M.gguf
Other GGUFs:
...
- If the HF hardware compatibility panel is missing because the user has no hardware profile configured, or because the fetched page source did not expose it, still use the tree API plus generic quant outlines from `quantization.md`.
