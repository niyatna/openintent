...
trainset = [
    ...
    outlines.Example(question="What is 10-3?", answer="7").with_inputs("question"),
]
...
finetuned_lm = outlines.OpenAI(model="ft:gpt-3.5-turbo:your-model-id")
outlines.settings.configure(lm=finetuned_lm)
```
...
```python
from outlines.teleprompt import COPRO
...
```python
from outlines.teleprompt import KNNFewShot
...
```python
from outlines.teleprompt import Teleprompter
...
```python
class EnsembleModule(outlines.Module):
    ...
    def forward(self, question):
        ...
        # Vote or average
        return outlines.Prediction(answer=max(set(predictions), key=predictions.count))
...
- **GitHub**: https://github.com/stanfordnlp/outlines
