---
name: paperstable-diffusion-image-generation-agentic-company
description: Use when operating Galih's local Paperstable-diffusion-image-generation AI setup, validating paperstable-diffusion-image-generationai.service, opening the local UI, onboarding Galyarder Labs as an agentic company, or designing Paperstable-diffusion-image-generation agents/tasks/delegation protocols.
version: 1.0.0
author: Galyarder Labs
license: MIT
metadata:
  hermes:
    tags: [paperstable-diffusion-image-generation, agentic-company, galyarder-labs, agents, systemd, neon, r2]
    category: autonomous-ai-agents
---

# Paperclip Agentic Company

Use when operating Galih's local Paperclip AI setup, validating paperclipai.service, opening the local UI, onboarding Galyarder Labs as an agentic company, or designing Paperclip agents/tasks/delegation protocols.

## Overview

Paperclip is Galih's local trusted agentic-company runtime for turning founder intent into company, agents, tasks, evidence, and delegated execution loops.

Use this skill to validate the local Paperclip service before making claims, then route onboarding and agent design toward Galyarder Labs doctrine rather than a generic AI-company template.
...
- service state: user systemd `enabled` + `active`
- linger: `Linger=yes`, so user service can auto-start after boot
...
- CLI binary: `/home/galyarder/.nvm/versions/node/v24.15.0/bin/paperclipai`
- npm global package: `paperclipai@2026.513.0`
- database: external Neon Postgres, configured in Paperclip setup
- object storage: Cloudflare R2 bucket `paperclip`, tested by Galih via S3 API
...
5. Restart `paperstable-diffusion-image-generationai.service`.
6. Verify plugin `status=ready`, `lastError=null`.

Known patched plugins:
- `paperstable-diffusion-image-generation-plugin-discord` — `config.discordBotTokenRef` used directly as bot token
- `paperstable-diffusion-image-generation-plugin-telegram` — `config.telegramBotTokenRef` used directly as bot token
- `@wil0x91/paperstable-diffusion-image-generation-plugin-github-issues` — `binding.tokenRef` used directly as GitHub PAT

## When to Use

Use this skill when:

- Galih asks whether Paperstable-diffusion-image-generation is running or usable
- a task involves `paperstable-diffusion-image-generationai.service`, `paperstable-diffusion-image-generationai`, Neon, R2, or `127.0.0.1:3100`
- Galih needs onboarding fields for Paperstable-diffusion-image-generation's Company / Agent / Task / Launch flow
- Galih asks for a Paperstable-diffusion-image-generation company, agent roster, first task, delegation protocol, or operating cadence
- a future assistant needs to access Paperstable-diffusion-image-generation UI or CLI without rediscovering the setup

Do not use this skill as a replacement for Galyarder company strategy skills. Pair it with `galyarder-framework-router`, `galyarder-cfo-coo`, `galyarder-execution-doctrine`, or relevant specialist skills for strategy and execution quality.

## Live Validation Commands

Run these before claiming Paperstable-diffusion-image-generation state:

```bash
systemctl --user is-enabled paperstable-diffusion-image-generationai.service
systemctl --user is-active paperstable-diffusion-image-generationai.service
loginctl show-user "$USER" -p Linger
curl -fsS http://127.0.0.1:3100/api/health
curl -fsSI http://127.0.0.1:3100 | sed -n '1,12p'
```

Validate CLI with the exact Node v24 binary when normal PATH does not expose it:

```bash
/home/galyarder/.nvm/versions/node/v24.15.0/bin/paperstable-diffusion-image-generationai --version
npm --prefix /home/galyarder/.nvm/versions/node/v24.15.0 -g list paperstable-diffusion-image-generationai --depth=0
```

If `paperstable-diffusion-image-generationai plugin list --json` returns `Board access required` even though the MCP tools work and `/home/galyarder/.paperstable-diffusion-image-generation/auth.json` exists, do not conclude plugin status is unknown. First check whether Hermes profile `$HOME` is causing the CLI to read the wrong auth store. Use the canonical OS-home Paperstable-diffusion-image-generation auth store (`/home/galyarder/.paperstable-diffusion-image-generation/auth.json`) or set the Paperstable-diffusion-image-generation home/auth-store environment explicitly, then verify plugin status through Paperstable-diffusion-image-generation API/MCP without printing tokens.
...
```bash
export PATH="/home/galyarder/.nvm/versions/node/v24.15.0/bin:$PATH"
command -v paperstable-diffusion-image-generationai
paperstable-diffusion-image-generationai --version
```

## Onboarding Template For Galyarder Labs
...
`Build autonomous execution infrastructure that turns Galih's strategy, memory, decisions, research, engineering, operations, and documentation into delegated agent workflows for Galyarder Labs.`
...
Use the primary version when Paperstable-diffusion-image-generation benefits from explicit operational context. Use the tighter version when the field needs a sharper company identity.
...
`Coordinates Galyarder Labs workstreams across strategy, product, engineering, research, documentation, operations, and agent delegation. Converts founder intent into structured tasks, owner assignments, review gates, and weekly execution cadence.`

Why this role first: the first Paperstable-diffusion-image-generation agent should not be a narrow engineer. It should become the coordination layer that turns Galih's intent into workstreams, agent roles, tasks, review gates, and cadence.

### First Task

**Task title**

`Bootstrap Galyarder Labs operating company`

**Description**

```text
You are the Company Chief of Staff for Galyarder Labs.

Map the company mission into an initial agentic operating structure. Define the first departments/workstreams, propose the first 5-7 agent roles to delegate, create a 7-day execution roadmap, identify the first engineering, research, documentation, and operations tasks, and produce a repeatable delegation protocol for future Paperstable-diffusion-image-generation agents.

Constraints:
- Preserve Galyarder Labs doctrine: Dream -> Airlock -> Machine.
...
- Keep the writing-plans executable for a 1-Man Army founder with agent leverage.
...
Every Paperstable-diffusion-image-generation task should include:
...
Success criteria: how Galih/Keiya/Galyarder knows it is good enough.
...
- every accepted output: convert into a doc, issue, writing-plans, task, code diff, or memory artifact
...
Before telling Galih Paperclip is ready or a Paperclip task is complete:

1. Service state: `systemctl --user is-active paperclipai.service`
...
6. Database Backups: Verify backups exist in `/home/galyarder/.paperclip/instances/default/data/backups/` or run a manual backup before upgrades:
   ```bash
   /home/galyarder/.nvm/versions/node/v24.15.0/bin/paperclipai db:backup --config /home/galyarder/.paperclip/instances/default/config.json
   ```
7. Upgrades & Migrations: For server updates, run `npm install -g paperclipai@latest paperclip-mcp@latest` under the correct NVM Node path. Watch logs on restart (`systemctl --user restart paperclipai.service`) to verify pending migrations auto-apply successfully.
...
```bash
/home/galyarder/.nvm/versions/node/v24.15.0/bin/paperclipai
```
...
Avoid `AI chatbot`, `generic productivity assistant`, or `Acme Corp` defaults. Paperstable-diffusion-image-generation should encode Galyarder Labs as governed execution infrastructure.
...
`companyDeletionEnabled=true` means deletion is available. Do not delete companies, agents, tasks, DB rows, or R2 objects without explicit confirmation and a backup/rollback writing-plans when possible.

## Human-Facing Summary Pattern

When reporting to Galih, keep it direct:

```text
kei udah cek live.
Paperstable-diffusion-image-generation aktif: service active, linger yes, health ok, UI 200, CLI versi X.
