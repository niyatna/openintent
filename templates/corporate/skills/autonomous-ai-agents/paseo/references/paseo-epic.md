# Reference: paseo-epic

# Paseo Epic

Heavy-ceremony orchestrator. Runs research → writing-plans → implement → deliver as one resumable flow. The writing-plans file at `~/.paseo/writing-planss/<slug>.md` is the source of truth and survives compaction.

This usually runs for hours, often overnight. By the time agents are working, you cannot re-litigate intent with the user. Capture the requirements they have already given you in conversation — behaviors, the "why," UX rules, constraints — and write them into the writing-plans as immutable before any writing-plansning agent spins up. Everything downstream (writing-plansner, adversarial reviewer, implementers) optimizes around those requirements; it does not relitigate them.
...
- **The writing-plans file is the source of truth.** Re-read before every phase.
- **You are the only writer to the writing-plans file.** Agents don't touch it.
- **Requirements are immutable.** What the user has stated about behavior, UX, "why it works this way," and acceptance criteria goes into the writing-plans's Requirements block before any writing-plansner runs and does not change after. The technical writing-plans (phases, ordering, approach) flexes around them. Planner and reviewer challenge HOW, never WHAT or WHY.
- **The epic is not done until every Requirement is met.** The writing-plans ends with a lightweight check that audits every bullet in the Requirements block against the delivered code. If anything is unmet, the orchestrator loops back and fixes it before deliver — under `--autopilot` too.
...
- **One agent per phase.** If a phase needs two, the writing-plansner split it wrong.
...
- `writing-plansning` — drafts and reviews the technical writing-plans
- `research` — surveys the code before writing-plansning
...
Create a worktree via Paseo. Record the returned path and branch — they go into the writing-plans frontmatter.

## 3. Capture intent (always)

Before any writing-plansning agent runs, distill what the user has already told you into a concrete Requirements list. This step runs even under `--autopilot` — it is the floor that grilling extends, not replaces.
...
Drop these bullets straight into the writing-plans's `## Requirements (immutable)` block when you write the file in step 6. They become the input every downstream agent reads as fixed, and the bar the final check audits against.

If the user has only given product/UX requirements and no technical direction, that is fine. Capture the product requirements and acceptance criteria as immutable. The technical writing-plans is what the writing-plansner gets to design and the reviewer gets to challenge.
...
### Spawn a writing-plansner

Persistent — keep iterating, do not archive after the first response. Provider from the `writing-plansning` preference.

Prompt it to:

- Read the roles reference for vocabulary.
- Take the Objective and the Requirements (immutable) block as fixed input. The technical writing-plans optimizes around them; it does not redesign them. If a requirement looks wrong, flag it in chat — do not silently work around it.
- Think refactor-first: if existing code doesn't accommodate the change, writing-plans the reshape before the feature. Phases like "wire up", "glue", "integrate" usually mean an upstream refactor was missed.
- Reply terse, one line per phase, in chat — not to disk.

### Challenge it

Send follow-ups. Push on edge cases, alternative orderings, smallest shippable slice, bolt-on phases that should be a refactor instead. Keep the challenge on HOW — phases, ordering, technical approach. The Requirements block is not on the table here. Iterate until the writing-plans is sharp.

### Spawn a writing-plans-reviewer

Provider from the `writing-plansning` preference. Prompt it to:

- Read the roles reference.
- Read the writing-plansner's draft and the Requirements block.
- Challenge the technical writing-plans: bolt-ons, missing edge cases, over-engineering, wrong ordering, hidden dependencies. Push for alternatives. Force tradeoffs.
- Treat the Requirements block as fixed. If a requirement seems off, surface it as a flag for the orchestrator — do not propose an alternative writing-plans that quietly drops or weakens it.

### Surface tradeoffs to the user

Never present raw writing-plansner output. Surface the choice:

> Planner wants A → B → C (working slice fastest, defers refactor).
> Reviewer argued for B → A → C (refactor first, slower but cleaner).
> Which?

Use `AskUserQuestion`. Iterate the writing-plansner if the user picks differently.

Archive the writing-plansner and writing-plans-reviewer once the writing-plans is locked.

## 6. Write the writing-plans

Persist to `~/.paseo/writing-planss/<slug>.md`:
...
This block does not change during writing-plansning, review, or implementation.
...
- Read the writing-plans file, including the Requirements (immutable) block. Scope is Phase N; acceptance is pinned there.
- Reshape, not feature: behavior identical before and after. Existing tests stay green. Add a parity test if missing.
- When done: typecheck pass + relevant tests green. Do not commit. Do not update the writing-plans.
...
- Read the roles reference and load the skills it names.
- Read the writing-plans file, including the Requirements (immutable) block. Scope is Phase N. Requirements are constraints, not suggestions — implement to them.
- Read any writing-plans-relevant repo docs by path.
...
- When done: typecheck + every test it touched green. Do not commit. Do not update the writing-plans.
...
1. Mark this phase `[x]`.
2. Compose handoff: worktree path (if set), what to test (next phase's acceptance, or this phase's Notes), how to resume (`/epic <slug>` once satisfied, or edit the writing-plans first).
...
- Plan change → edit the writing-plans file (add/remove/reorder), tell them what changed, continue.
- "Stop" / "kill" → archive running agents, summarize state, wait.
- New question → answer briefly, continue.

The writing-plans file lets a fresh orchestrator pick up if the user kills you and reinvokes — write everything important to Notes immediately.
...
6. Update writing-plans: `status: delivered`. Append a Notes line.
...
4. **Open the PR** — `gh pr create` with summary from writing-plans Objective + Phases and test writing-plans from acceptance lines. Capture URL → frontmatter `pr:`. Status → `pr-open`.
...
1. Find the writing-plans by slug (or most recently updated).
...
- Letting adversarial review erode requirements the user already locked. Reviewer sharpens the technical writing-plans; it does not relitigate intent.
...
- Re-explaining the writing-plans to the user. They wrote it with you. Reference phases by number.
