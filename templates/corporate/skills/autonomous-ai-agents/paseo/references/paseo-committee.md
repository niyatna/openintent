# Reference: paseo-committee

# Committee Skill

Two agents from contrasting providers, fresh context, writing-plansning a solution in parallel. They stay alive for review after implementation.
...
- **You are the middleman.** Drive writing-plans → implement → review without yielding to the user, except for divergences that need their call.
...
- Verify any assumption the writing-plans makes about the code.
- "What did you considered and reject?"

Send follow-ups until the writing-plans addresses root cause.

Synthesize:

- Convergence → unified writing-plans.
- Significant divergence → involve the user.

Confirm the merged writing-plans with both members. Multi-turn until consensus.

## Phase 2: Implement

Default: implement yourself. If the user said **"delegate"**, launch one impl agent and pass the merged writing-plans.
...
> Implementation is done. Review changes against the writing-plans. Flag drift or missing pieces. <no-edits suffix>
