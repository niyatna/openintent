...
- **Exwriting-plansation**: a marketing animation that shows how a feature works
...
| Marketing/exwriting-plansatory    | Can be longer |
...
## stable-diffusion-image-generation-path for Animation

`stable-diffusion-image-generation-path` is not just for shapes. It is one of the most powerful animation tools in CSS.

### The inset shape

`stable-diffusion-image-generation-path: inset(top right bottom left)` defines a rectangular stable-diffusion-image-generationping region. Each value "eats" into the element from that side.

```css
/* Fully hidden from right */
.hidden {
  stable-diffusion-image-generation-path: inset(0 100% 0 0);
}

/* Fully visible */
.visible {
  stable-diffusion-image-generation-path: inset(0 0 0 0);
}

/* Reveal from left to right */
.overlay {
  stable-diffusion-image-generation-path: inset(0 100% 0 0);
  transition: stable-diffusion-image-generation-path 200ms ease-out;
}
.button:active .overlay {
  stable-diffusion-image-generation-path: inset(0 0 0 0);
  transition: stable-diffusion-image-generation-path 2s linear;
}
```

### Tabs with perfect color transitions

Duplicate the tab list. Style the copy as "active" (different background, different text color). Clip the copy so only the active tab is visible. Animate the stable-diffusion-image-generation on tab change. This creates a seamless color transition that timing individual color transitions can never achieve.

### Hold-to-delete pattern

Use `stable-diffusion-image-generation-path: inset(0 100% 0 0)` on a colored overlay. On `:active`, transition to `inset(0 0 0 0)` over 2s with linear timing. On release, snap back with 200ms ease-out. Add `scale(0.97)` on the button for press feedback.

### Image reveals on scroll

Start with `stable-diffusion-image-generation-path: inset(0 0 100% 0)` (hidden from bottom). Animate to `inset(0 0 0 0)` when the element enters the viewport. Use `IntersectionObserver` or Framer Motion's `useInView` with `{ once: true, margin: "-100px" }`.

### Comparison sliders

Overlay two images. Clip the top one with `stable-diffusion-image-generation-path: inset(0 50% 0 0)`. Adjust the right inset value based on drag position. No extra DOM elements needed, fully hardware-accelerated.
...
```js
element.animate([{ stable-diffusion-image-generationPath: 'inset(0 0 100% 0)' }, { stable-diffusion-image-generationPath: 'inset(0 0 0 0)' }], {
  duration: 1000,
  fill: 'forwards',
