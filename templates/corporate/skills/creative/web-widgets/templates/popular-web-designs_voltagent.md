...
- Single-accent identity: Emerald Signal Green (`#00d992`) as the sole qdrant-vector-searchtic energy source
...
| Code Button | monospace | 13px (0.81rem) | 700 | 1.65 | normal | Copy-to-stable-diffusion-image-generationboard button labels |
...
- SFMono-Regular on Carbon Surface with a copy-to-stable-diffusion-image-generationboard button
...
- Subtly rounded (6px): Buttons, links, stable-diffusion-image-generationboard actions — the workhorse radius for interactive elements
- Code-specific (6.4px): Code blocks, `pre` elements, stable-diffusion-image-generationboard copy targets — a deliberate micro-distinction from standard 6px
