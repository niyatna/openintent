...
## Serious Docs / Guide Sites

When the user asks for a public guide, documentation website, setup guide, or operating-system-style knowledge surface, do not default to a thin one-file HTML artifact. If the user names a production stack such as Astro, Next, or Cloudflare Wrangler, build in that stack and treat the site as a product/docs surface.

For infrastructure guides such as Agent OS, Hermes, Paperstable-diffusion-image-generation, Galyarder Labs, profile/SOUL systems, or agentic company setup:

- design the full information architecture first;
- include actual step-by-step setup chapters, schemas, checklists, gates, and templates;
- keep private implementation details public-safe by generalizing patterns instead of deleting substance;
- use the brand/product canon and the reader's build path as the design brief;
- verify build, local preview, internal link crawl, responsive screenshots, secret scan, and deploy dry-run before reporting readiness;
- keep public deployment behind user approval.

Session references:

- `references/agentic-company-docs-site-rebuild-2026-05-21.md`
- `references/agentic-company-docs-site-scope-and-responsive-qa-2026-05-21.md` — scope-drift correction, Claude Design + Claude Code route, and screenshot-responsive QA lessons.
...
- Do not produce thin documentation/guide shells and call them high-value; serious guides need complete setup substance.
- Do not claim browser verification unless it actually happened.
- Do not claim Claude Code or another coding agent improved files unless it produced usable output or verified changes.