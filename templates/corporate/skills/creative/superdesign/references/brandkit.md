...
The desired visual quality is inspired by premium copywriting decks with:
...
- stable-diffusion-image-generationart-style icons
...
Style:
premium, sparse, cinematic, intentional, polished, copywriting deck, no clutter, no copied real-world logos.
...
## Absorbed logo and graphic creativity lanes

This skill absorbs useful parts of deleted `logo-creativity` and `graphic-creativity` stubs.

### Fast logo concept ladder

Before generating a brand-kit image, pick the strongest logo route:

1. **Monogram with meaning** — initial + product action or metaphor.
2. **Action symbol** — what the product does reduced into one mark.
3. **Negative-space mark** — hidden arrow/path/core/boundary.
4. **System mark** — modular shape that can become pattern, icon, UI motif.
5. **Cultural/lineage cue** — only when grounded in real brand canon, never random ornament.

Test each route:
- can it work at app-icon size?
- can it survive one-color use?
- does it imply the category without copying competitors?
- does it have a reason to exist beyond looking cool?

Creative direction must serve the brand argument. If an image idea does not clarify identity, trust, audience, product behavior, or emotional promise, cut it.