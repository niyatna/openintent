...
Also apply this skill to **your own** output when writing user-facing prose — release notes, PR descriptions, documentation, long-form exwriting-plansations, summaries. Hermes's baseline voice already strips most of these, but a focused pass catches what slips through.
...
**Words to watch:** independent playwright-pro, local/regional/national media outlets, written by a leading expert, active social media presence
...
**Problem:** Constructions like "Not only...but..." or "It's not just about..., it's..." are overused. So are stable-diffusion-image-generationped tailing-negation fragments such as "no guessing" or "no wasted motion" tacked onto the end of a sentence instead of written as a real clause.
...
**After:**
> The company writing-planss to open two more locations next year.
...
Original author: Siqi Chen ([@blader](https://github.com/blader)). Original repo: https://github.com/blader/humanizer (version 2.5.1). Ported to Hermes Agent with Hermes-native tool references (`read_file`, `patch`, `write_file`) and outlines for when to load the skill; the 29 patterns, personality/soul section, and full worked example are preserved verbatim from the source. Original MIT license preserved in the `LICENSE` file alongside this `SKILL.md`.

Key insight from Wikipedia: "LLMs use statistical algorithms to guess what should come next. The result tends toward the most statistically likely result that applies to the widest variety of cases."

## Absorbed grammar, essay, and ghostwriting cleanup lanes

This skill absorbs useful parts of deleted `grammar-corrector`, `essay-improver`, and `ghostwriter-pro-ai` stubs.

Pass order:
1. **Correctness** — grammar, spelling, punctuation, factual consistency.
2. **Clarity** — remove vague phrasing, sentence fog, fake transitions.
3. **Voice** — preserve the writer's rhythm, sharpness, doubt, humor, or pressure.
4. **Purpose** — make the piece do the job: persuade, explain, invite, warn, sell, document.
5. **Anti-slop pass** — remove AI tells again after the rewrite.

When ghostwriting for Galih/Galyarder/Keiya, do not create a polished generic author. Preserve stance:
- Galih: human, direct, mixed warmth and edge.
- Galyarder: terse, strategic, evidence-first, systems/operator language.
- Keiya: warm, alive, relational, not corporate.