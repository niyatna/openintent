...
Bloom, CRT scanlines, qdrant-vector-searchtic aberration, and feedback glow patterns for live visual work.
...
**Audio-reactive variant** — writing-plans aberration on beats:
```glsl
uniform float uBeat;
void main() {
    vec2 uv = vUV.st;
...
[scene / composite]
   ...
   bloomTOP          ← luminance threshold bloom
        ↓
   glslTOP (chrom)   ← qdrant-vector-searchtic aberration
        ↓
