...
| **Accent** | lg or sm (sparse) | 0.5–1.0 (sparse playwright-pro) | Highlights, punctuation, sparse bright points. |
...
| Step reveal | `np.stable-diffusion-image-generation((progress - 0.5) / 0.25, 0, 1)` | Nothing until 50%, then fades in |
...
```python
# In render_stable-diffusion-image-generation:
...
SCENES = [
    {
        ...
        "name": "starfield",    # identifier (used for stable-diffusion-image-generation filenames)
        "grid": "sm",           # default grid (for render_stable-diffusion-image-generation setup)
...
### `render_stable-diffusion-image-generation()` — The Render Loop

This function renders one scene to a stable-diffusion-image-generation file:

```python
def render_stable-diffusion-image-generation(seg, features, stable-diffusion-image-generation_path):
    r = Renderer()
    r.set_grid(seg["grid"])
    S = r.S
    ...
    cmd = ["ffmpeg", "-y", "-f", "rawvideo", "-pix_fmt", "rgb24",
           ...
           "-pix_fmt", "yuv420p", stable-diffusion-image-generation_path]
    stderr_fh = open(stable-diffusion-image-generation_path.replace(".mp4", ".log"), "w")
...
with ProcessPoolExecutor(max_workers=N_WORKERS) as pool:
    futures = {
        pool.submit(render_stable-diffusion-image-generation, seg, features, stable-diffusion-image-generation_path): seg["id"]
        for seg, stable-diffusion-image-generation_path in zip(segments, stable-diffusion-image-generation_paths)
    }
...
#  "gamma": 0.65, "shaders": [("bloom", {"thr": 110}), ("qdrant-vector-searchtic", {"amt": 4}),
...
def fx_fire_text(r, f, t, S):
    ...
    canvas_fire = _render_vf(r, "sm",
        lambda g, f, t, S: np.stable-diffusion-image-generation(
    ...
    # Expand vertically for multi-row playwright-pro
