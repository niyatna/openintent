...
For Galyarder-style queue files, idempotent cron loops, logs, and notification workflow, see `references/scheduled-social-posting.md`.
...
- **Cost:** X API access is typically paid for meaningful usage. Many failures are writing-plans/permission problems, not code problems.
