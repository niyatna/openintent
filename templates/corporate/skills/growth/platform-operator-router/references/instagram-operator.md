...
Default route is direct CloakBrowser persistent-profile frontend execution from a saved session. Camofox is a legacy/fallback route while existing proven scripts are still Camofox-based or CloakBrowser is blocked. No UI discovery while holding a live/public action.

Fast path:

1. Pick target account: `keiya` or `galyarder`; use only that account's Instagram paths.
...
Stop rule: if owner-state is absent, media preview is absent, caption counter stays `0/2,200`, a modal blocks Share/Create, or the visible menu is not the original post menu, stop before posting/deleting. Patch/reference the trap, then retry. Do not live-post first and repair later unless Galih explicitly asks for emergency repair.

Runtime: if using legacy Camofox scripts, prefer Node v24 (`/home/galyarder/.nvm/versions/node/v24.15.0/bin/node`) when using `camoufox-js`; Node v26 can break native modules such as `better-sqlite3`.
...
1. Load account/cookie state from `/home/galyarder/.hermes/private/credentials/agents/<agent>/instagram/`.
2. Open the dedicated CloakBrowser persistent profile first; use Camofox cookie import only as fallback.
...
5. If login gate remains, do CloakBrowser login/TOTP first; use Camofox login/TOTP fallback only if CloakBrowser is blocked, then save/update cookies and retest owner-state.

## Absolute rule

For Galih's dedicated agent-owned Instagram accounts, the workflow is **CloakBrowser persistent profile first** for login/upload/posting. Camofox is legacy/fallback only while proven scripts remain Camofox-based or CloakBrowser is blocked. No API-first route. No Hermes native browser. No random Brave route.

Public/destructive actions require exact Galih approval unless he gave exact action in the current turn. Never print secrets/cookies/tokens.
...
/home/galyarder/.hermes/private/browser-profiles/agents/<agent>/instagram-cloakbrowser/
/home/galyarder/.hermes/private/browser-profiles/agents/<agent>/instagram-camofox/  # legacy fallback
```

Known handles:

- Keiya: `keiyazeyniputri`
- Galyarder: `galyarderlabs.ai`
...
- observed Camofox controls: `Instagram`, `Home`, `Reels`, `Messages`, `Search`, `Explore`, `Notifications`, `New post`, `Settings`
- Galyarder business account may also show `Professional dashboard`
- `Edit profile`
- `View archive`
...
- `Save login info` -> usually `Not now` unless Galih requested persistence change
...
6. Use default crop/settings unless Galih specified otherwise.
...
5. Handle `Save login info` / notifications prompts conservatively (`Not now`) unless Galih specified otherwise.
6. Save/update cookies; rerun owner-state check.

## References

- `references/instagram-camofox-posting-caption-edit-2026-05-18.md` — live Keiya Instagram posting lesson: compact Create control, notification modal, media preview, caption field targeting, and owner-menu caption repair.
