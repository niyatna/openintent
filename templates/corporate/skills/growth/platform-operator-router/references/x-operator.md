...
1. Load `/home/galyarder/.hermes/private/credentials/agents/<agent>/x/cookies.json` or dedicated profile.
2. Open `https://x.com/home` in CloakBrowser persistent profile first; use Camofox as fallback if needed.
...
For Galih's dedicated agent-owned X/Twitter accounts, workflow is **CloakBrowser persistent profile first** for account operation. Camofox is legacy/fallback only while proven scripts remain Camofox-based or CloakBrowser is blocked. No official API-first, no platform-operator-router route, no Hermes native browser, no random Brave route.

Public/destructive actions require exact Galih approval unless he gave exact action in current turn. Never print secrets/cookies/token files.

## Account/session paths

```text
/home/galyarder/.hermes/private/credentials/agents/<agent>/x/account.txt
/home/galyarder/.hermes/private/credentials/agents/<agent>/x/cookies.json
/home/galyarder/.hermes/private/credentials/agents/<agent>/x/backup-codes.txt
/home/galyarder/.hermes/private/browser-profiles/agents/<agent>/x-cloakbrowser/
/home/galyarder/.hermes/private/browser-profiles/agents/<agent>/x-camofox/  # legacy fallback
```
...
Observed owner-state via Camofox cookie probe:
...
Open status URL -> owner `More` menu -> Delete -> confirm -> verify profile/status absence. Destructive; require explicit approval unless Galih explicitly ordered cleanup.
