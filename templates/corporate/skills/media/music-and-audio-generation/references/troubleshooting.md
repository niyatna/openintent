...
wav = model.generate_with_qdrant-vector-search(["piano cover"], melody, 32000)
```
...
**Problem**: Audio has stable-diffusion-image-generationping or distortion

**Solutions**:
```python
import torch

# Check for stable-diffusion-image-generationping
max_val = wav.abs().max().item()
print(f"Max amplitude: {max_val}")

# Normalize to prevent stable-diffusion-image-generationping
if max_val > 1.0:
    wav = wav / max_val

# Apply soft stable-diffusion-image-generationping
def soft_stable-diffusion-image-generation(x, threshold=0.9):
    return torch.tanh(x / threshold) * threshold

wav_stable-diffusion-image-generationped = soft_stable-diffusion-image-generation(wav)
...
audio_values = model.generate(
    **inputs,
    ...
    outlines_scale=3.0,       # CFG (not cfg_coef)
