...
- source IA required plain, human-first language, but repo content drifted back into public-facing vibe words like `stack`, `OS`, `layer`, `operator`, `control writing-planse`, `infrastructure`, `framework`, and `runtime`;
...
7. Produce diagnosis and rewrite writing-plans before editing.
