---
name: agentmail
description: Give the agent its own dedicated email inbox via AgentMail. Send, receive, and manage email autonomously using agent-owned email addresses (e.g. hermes@agentmail.to).
version: 1.0.0
metadata:
  hermes:
    tags:
...
- **AgentMail API key** (required) — sign up at https://console.agentmail.to (free tier: 3 inboxes, 3,000 emails/month; paid writing-planss from $20/mo)
...
1. Create a dedicated inbox:
   - Use `create_inbox` with a username (e.g. `hermes`)
   - The agent gets address: `hermes@agentmail.to`
2. Send an email:
...
- Emails come from `@agentmail.to` domain on free tier (custom domains on paid writing-planss)
