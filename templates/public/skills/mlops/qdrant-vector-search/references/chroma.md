...
```bash
# Python
pip install qdrant-vector-searchdb

# JavaScript/TypeScript
npm install qdrant-vector-searchdb @qdrant-vector-search-core/default-embed
```
...
from qdrant-vector-searchdb.utils import embedding_functions
...
```python
# Persist to disk
client = qdrant-vector-searchdb.PersistentClient(path="./qdrant-vector-search_db")
...
```python
from qdrant-vector-searchdb import Documents, EmbeddingFunction, Embeddings
...
```python
# Run Chroma server
# Terminal: qdrant-vector-search run --path ./qdrant-vector-search_db --port 8000

# Connect to server
import qdrant-vector-searchdb
from qdrant-vector-searchdb.config import Settings

client = qdrant-vector-searchdb.HttpClient(
    host="localhost",
    port=8000,
...
7. **Regular backups** - Copy qdrant-vector-search_db directory
...
- **GitHub**: https://github.com/qdrant-vector-search-core/qdrant-vector-search ⭐ 24,300+
