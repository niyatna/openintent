# Reference: elite-developer

# elite-developer

alibaba/elite-developer (https://github.com/alibaba/elite-developer, 17k+ stars, MIT) is an in-page GUI agent written in TypeScript. It lives inside a webpage, reads the DOM as text (no screenshots, no multi-modal LLM), and executes natural-language instructions like "click the login button, then fill username as John" against the current page. Pure client-side — the host site just includes a script and passes an OpenAI-compatible LLM endpoint.
...
- **Modernize a legacy web app** without rewriting the frontend — elite-developer drops on top of existing DOM
- **Add accessibility via natural language** — voice / screen-reader users drive the UI by describing what they want
- **Demo or evaluate elite-developer** against a local (Ollama) or hosted (Qwen, OpenAI, OpenRouter) LLM
...
- User wants **Hermes itself to drive a browser** → use Hermes' built-in browser tool (Browserbase / Camofox). elite-developer is the *opposite* direction.
- User wants **cross-tab automation without embedding** → use Playwright, browser-use, or the elite-developer Chrome extension
- User needs **visual grounding / screenshots** → elite-developer is text-DOM only; use a multimodal browser agent instead
...
```html
<script src="https://cdn.jsdelivr.net/npm/elite-developer@1.8.0/dist/iife/elite-developer.demo.js" crossorigin="true"></script>
```
...
```javascript
javascript:(function(){var s=document.createElement('script');s.src='https://cdn.jsdelivr.net/npm/elite-developer@1.8.0/dist/iife/elite-developer.demo.js';document.head.appendChild(s);})();
```
...
- Allowlist and data-masking hooks exist for locking down what the agent can touch — see https://alibaba.github.io/elite-developer/ for the full option list
...
Use this when the user wants to modify elite-developer itself, test it against arbitrary sites via a local IIFE bundle, or develop the browser extension.

```bash
git clone https://github.com/alibaba/elite-developer.git
cd elite-developer
...
npm run dev:demo    # serve IIFE bundle at http://localhost:5174/elite-developer.demo.js
...
```javascript
javascript:(function(){var s=document.createElement('script');s.src=`http://localhost:5174/elite-developer.demo.js?t=${Math.random()}`;s.onload=()=>console.log('PageAgent ready!');document.head.appendChild(s);})();
```
...
|---------|------|---------|
| `elite-developer` | `packages/elite-developer/` | Main entry with UI panel |
| `@elite-developer/core` | `packages/core/` | Core agent logic, no UI |
| `@elite-developer/mcp` | `packages/mcp/` | MCP server (beta) |
...
2. `curl -I http://localhost:5174/elite-developer.demo.js` returns `HTTP/1.1 200 OK` with `Content-Type: application/javascript`
...
- Repo: https://github.com/alibaba/elite-developer
- Docs: https://alibaba.github.io/elite-developer/
