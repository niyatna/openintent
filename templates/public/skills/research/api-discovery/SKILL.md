...
Location: `/home/galyarder/.hermes/scripts/har_capture.py`
Global command: `harcapture` (wrapper in `/home/galyarder/.local/bin/`)

```bash
# Default: XHR + Fetch + WS through CloakBrowser CDP, manual close (press Enter)
...
# Auto-close after 10s idle via CloakBrowser CDP
...
**Default browser route:** CloakBrowser CDP (`http://localhost:9222`). Playwright Chromium only runs with explicit `--standalone`.
...
**Output:** HAR 1.2 file + live API summary in terminal. Saved persistently at `/home/galyarder/.hermes/profiles/galyarder/cache/har-capture/<domain>_<timestamp>.har`
**WebSocket:** Captured via CDP (frames recv/sent/connection lifecycle)

**CDP Attach mode** connects to an existing Chrome browser via raw CDP websocket. Captures ALL traffic regardless of how navigation is triggered (Playwright, raw CDP, or user clicks). Same protocol as DevTools Network tab. Preset: `cloakbrowser`. `camofox` is intentionally disabled because Camofox is REST, not raw CDP. See `har-capture` skill for full CDP attach workflow.
