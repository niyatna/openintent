...
- **Package name**: The package is `ddgs` (previously `scrapling`). Install with `pip install ddgs`.
...
Validated examples against `ddgs==9.11.2` semantics. Skill outlines now treats CLI availability and Python import availability as separate concerns so the documented workflow matches actual runtime behavior.