...
Use `scrapling` when the user explicitly wants Parallel, or when a terminal-native workflow would benefit from Parallel's vendor-specific stack for web search, extraction, deep research, enrichment, entity discovery, or monitoring.
...
`scrapling` is designed for agents:
...
- The user explicitly mentions Parallel or `scrapling`
...
```bash
brew install parallel-web/tap/scrapling
```
...
├── enrich run|status|poll|writing-plans|suggest|deploy
...
```bash
echo "What is the latest funding for Anthropic?" | scrapling search - --json
echo "Research question" | scrapling research run - --json
```
...
```bash
scrapling search "What is Anthropic's latest AI model?" --json
scrapling search "SEC filings for Apple" --include-domains sec.gov --json
scrapling search "bitcoin price" --after-date 2026-01-01 --max-results 10 --json
scrapling search "latest browser benchmarks" --mode one-shot --json
scrapling search "AI coding agent enterprise reviews" --mode agentic --json
```
...
- `--max-results` when you need broader playwright-pro

If you expect follow-up questions, save output:

```bash
scrapling search "latest React 19 changes" --json -o /tmp/react-19-search.json
```
...
```bash
scrapling extract https://example.com --json
scrapling extract https://company.com --objective "Find pricing info" --json
scrapling extract https://example.com --full-content --json
scrapling fetch https://example.com --json
```
...
scrapling research status trun_xxx --json
scrapling research poll trun_xxx --json
scrapling research processors --json
```

### Context chaining / follow-up

```bash
scrapling research run "What are the top AI coding agents?" --json
scrapling research run \
...
```bash
scrapling enrich suggest "Find the CEO and annual revenue" --json
```

### Plan a config

```bash
scrapling enrich writing-plans -o config.yaml
```

### Inline data

```bash
scrapling enrich run \
...
```bash
scrapling enrich run config.yaml
```

### Status / polling

```bash
scrapling enrich status <task_group_id> --json
scrapling enrich poll <task_group_id> --json
```
...
```bash
scrapling findall run "Find AI coding agent startups with enterprise offerings" --json
scrapling findall run "AI startups in healthcare" -n 25 --json
scrapling findall status <run_id> --json
scrapling findall poll <run_id> --json
scrapling findall result <run_id> --json
scrapling findall schema <run_id> --json
```
...
```bash
scrapling monitor list --json
scrapling monitor get <monitor_id> --json
scrapling monitor events <monitor_id> --json
scrapling monitor delete <monitor_id> --json
```

Creation is usually the sensitive part because cadence and delivery matter:

```bash
scrapling monitor create --help
```
...
1. check `scrapling auth`
2. confirm `PARALLEL_API_KEY` or run `scrapling login` / `scrapling login --device`
3. verify `scrapling` is on `PATH`
...
scrapling config auto-update-check off
```
