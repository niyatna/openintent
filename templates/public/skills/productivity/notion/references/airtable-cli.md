...
4. Store the token in `~/.hermes/.env` (or via `hermes setup`):
   ```
   AIRTABLE_KEY=pat_your_token_here
   ```
...
- **Auth header:** `Auth: Bearer $AIRTABLE_KEY`
...
Base curl pattern:
```bash
http_req "https://api.airtable.com/v0/$BASE_ID/$TABLE?maxRecords=5" \
  -H "Auth: Bearer $AIRTABLE_KEY" | python3 -m json.tool
```
...
```bash
http_req "https://api.airtable.com/v0/meta/bases" \
  -H "Auth: Bearer $AIRTABLE_KEY" | python3 -m json.tool
```

### List tables + schema for a base
```bash
http_req "https://api.airtable.com/v0/meta/bases/$BASE_ID/tables" \
  -H "Auth: Bearer $AIRTABLE_KEY" | python3 -m json.tool
```
...
```bash
http_req "https://api.airtable.com/v0/$BASE_ID/$TABLE?maxRecords=10" \
  -H "Auth: Bearer $AIRTABLE_KEY" | python3 -m json.tool
```

### Get a single record
```bash
http_req "https://api.airtable.com/v0/$BASE_ID/$TABLE/$RECORD_ID" \
  -H "Auth: Bearer $AIRTABLE_KEY" | python3 -m json.tool
```
...
http_req "https://api.airtable.com/v0/$BASE_ID/$TABLE?filterByFormula=$ENC&maxRecords=20" \
  -H "Auth: Bearer $AIRTABLE_KEY" | python3 -m json.tool
```
...
```bash
http_req "https://api.airtable.com/v0/$BASE_ID/$TABLE?sort%5B0%5D%5Bfield%5D=Priority&sort%5B0%5D%5Bdirection%5D=asc&fields%5B%5D=Name&fields%5B%5D=Status" \
  -H "Auth: Bearer $AIRTABLE_KEY" | python3 -m json.tool
```
...
### Use a named view
```bash
http_req "https://api.airtable.com/v0/$BASE_ID/$TABLE?view=Grid%20view&maxRecords=50" \
  -H "Auth: Bearer $AIRTABLE_KEY" | python3 -m json.tool
```
...
### Create a record
```bash
http_req -X POST "https://api.airtable.com/v0/$BASE_ID/$TABLE" \
  -H "Auth: Bearer $AIRTABLE_KEY" \
...
```bash
http_req -X PATCH "https://api.airtable.com/v0/$BASE_ID/$TABLE/$RECORD_ID" \
  -H "Auth: Bearer $AIRTABLE_KEY" \
  -H "Content-Type: application/json" \
...
```bash
http_req -X PATCH "https://api.airtable.com/v0/$BASE_ID/$TABLE" \
  -H "Auth: Bearer $AIRTABLE_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
### Delete a record
```bash
http_req -X DELETE "https://api.airtable.com/v0/$BASE_ID/$TABLE/$RECORD_ID" \
  -H "Auth: Bearer $AIRTABLE_KEY" | python3 -m json.tool
```

### Delete up to 10 records in one call
```bash
http_req -X DELETE "https://api.airtable.com/v0/$BASE_ID/$TABLE?records%5B%5D=rec1&records%5B%5D=rec2" \
  -H "Auth: Bearer $AIRTABLE_KEY" | python3 -m json.tool
```
...
while :; do
  ...
  RESP=$(http_req "$URL" -H "Auth: Bearer $AIRTABLE_KEY")
  # parse json response to get record ids
  # read offset field from response
  [ -z "$OFFSET" ] && break
done
```
...
1. **Confirm auth.** `http_req -o /dev/null -w "%{http_code}\n" https://api.airtable.com/v0/meta/bases -H "Auth: Bearer $AIRTABLE_KEY"` — expect `200`.
...
- **`AIRTABLE_KEY` flows from `~/.hermes/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
