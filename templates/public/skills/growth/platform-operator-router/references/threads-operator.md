...
```text
1. identify agent: keiya | galyarder
...
Verified behavior from current references: Keiya and Galyarder Threads cookies can open owner-state without login through this script.

## Absolute rule

For Galih's dedicated agent-owned Threads accounts, the workflow is **CloakBrowser persistent profile first**. Camofox remains a legacy/fallback route while this platform's proven frontend scripts are still Camofox-based or CloakBrowser is blocked.
No official API-first route. No Hermes native browser route. No random Brave route.

Mandatory chain:

```text
platform-operator-router -> CloakBrowser persistent profile first -> Camofox fallback only when needed -> verify owner-state -> requested CRUD -> verify live
```

Public/destructive actions require exact Galih approval unless he gave the exact action in the current turn.
...
/home/galyarder/.hermes/private/browser-profiles/agents/<agent>/threads-cloakbrowser/
/home/galyarder/.hermes/private/browser-profiles/agents/<agent>/threads-camofox/  # legacy fallback
```
...
Observed owner-state on Keiya via Camofox cookie probe:
...
5. For media: attach via Camofox frontend file input/file chooser.
...
Reply, repost/rethread, like, follow/unfollow, DM, and profile changes are public actions. Execute only after exact Galih approval/scope. Verify final visible state.
...
- `scripts/threads_cookie_reuse_smoke.py` — imports old `cookies.json` into a fresh Camofox session, opens `https://www.threads.com/login`, checks owner-state vs login gate, and prints sanitized JSON.
...
- `references/threads-camofox-media-posting-2026-05-17.md` — media posting, caption correction, deletion menu trap.
