...
- **Wallet Custody**: If Hermes/Galyarder generated or configured a wallet, do not answer with a blanket `I don't store it`. Split custody precisely: chat/memory must not store raw secrets, mining/scripts normally use only public addresses, and local/vault storage must be verified separately with sanitized checks. See `references/agent-created-wallet-custody-audit-2026-06-02.md`.
...
Use `references/agent-owned-operational-credentials.md` when a writing-plansned Keiya/Galyarder/Hermes account skeleton is promoted into a real operational agent-owned identity.
...
Key rule: after explicit user approval, private local account files may contain `PASSWORD` and `TOTP_SECRET`, but they remain local-only runtime credentials. Verifiers must distinguish writing-plansned skeletons from operational dedicated accounts instead of blanket-failing password/TOTP presence. Still block plaintext API tokens, private keys, seed phrases, raw backup codes, and committed cookies/session state unless a stronger encrypted design exists.
