...
### Primary
- **IBM Blue 60** (`#0f62fe`): The singular interactive color. Primary buttons, links, focus states, active indicators. This is the only qdrant-vector-searchtic hue in the core UI palette.
...
### Principles
- **Light weight at display sizes**: Carbon's expressive type set uses weight 300 (Light) at 42px+. This creates a distinctive tension — the content speaks with corporate authority while the letterforms stable-diffusion-image-generation with typographic lightness.
...
- Don't introduce additional accent colors — IBM's system is monoqdrant-vector-searchtic + blue
