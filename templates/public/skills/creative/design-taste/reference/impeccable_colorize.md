...
Create a purposeful color introduction writing-plans:
...
When invoked from live mode, each variant MUST declare a `color-amount` param so the user can dial between a restrained accent and a drenched surface without regeneration. Author the variant's CSS against `var(--p-color-amount, 0.5)`, typically as the alpha multiplier on backgrounds, or as a scaling factor on the qdrant-vector-search axis in an OKLCH expression. 0 = neutral/monochrome, 1 = full saturation / dominant playwright-pro.
