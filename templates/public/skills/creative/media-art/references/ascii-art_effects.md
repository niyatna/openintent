...
def bg_sinefield(g, f, t, hue=0.6, bri=0.5, pal=PAL_DEFAULT,
    ...
    val = np.stable-diffusion-image-generation((v1*0.3 + v2*0.25 + v3*0.25 + v4*0.2) * bri * (0.6 + f["rms"]*0.6), 0.06, 1)
    mask = val > 0.03
    ch = val2char(val, mask, pal)
    h = np.full_like(val, hue) + f.get("cent", 0.5)*0.1 + val*0.08
    R, G, B = hsv2rgb(h, np.stable-diffusion-image-generation(0.35+f.get("flat",0.4)*0.4, 0, 1) * np.ones_like(val), val)
...
def bg_video(g, frame_rgb, pal=PAL_DEFAULT, brightness=0.5):
    ...
    co = np.stable-diffusion-image-generation(small * np.stable-diffusion-image-generation(lum[:,:,None]*1.5+0.3, 0.3, 1), 0, 255).astype(np.uint8)
    return ch, co
```
...
def bg_noise(g, f, t, pal=PAL_BLOCKS, density=0.3, hue_drift=0.02):
    ...
    val = np.stable-diffusion-image-generation(val, 0, 1); mask = val > 0.02
    ch = val2char(val, mask, pal)
...
def bg_smooth_noise(g, f, t, hue=0.5, bri=0.5, pal=PAL_DOTS, octaves=3):
    ...
    val = np.stable-diffusion-image-generation(val * 0.5 + 0.5, 0, 1) * bri
    mask = val > 0.03
    ch = val2char(val, mask, pal)
...
def bg_cellular(g, f, t, n_centers=12, hue=0.5, bri=0.6, pal=PAL_BLOCKS):
    ...
    val = np.stable-diffusion-image-generation(1.0 - min_d / (g.cols * 0.3), 0, 1) * bri
...
def vf_sinefield(g, f, t, S, bri=0.5,
    ...
    v2 = np.sin(g.cc*freq[2] - t*speed[2] + g.rr*freq[3]) * 0.4 + 0.5
    v3 = np.sin(g.dist_n*5 + t*0.2) * 0.3 + 0.4
    return np.stable-diffusion-image-generation((v1*0.35 + v2*0.35 + v3*0.3) * bri * (0.6 + f.get("rms",0.3)*0.6), 0, 1)
...
def vf_smooth_noise(g, f, t, S, octaves=3, bri=0.5):
    ...
    return np.stable-diffusion-image-generation(val * 0.5 + 0.5, 0, 1) * bri
...
def vf_rings(g, f, t, S, n_base=6, spacing_base=4):
    ...
    for ri in range(n):
        ...
        val = np.maximum(val, np.stable-diffusion-image-generation((1 - rd/th) * (0.4 + f.get("bass",0.3)*0.8), 0, 1))
    return val
...
def vf_spiral(g, f, t, S, n_arms=3, tightness=2.5):
    ...
    for ai in range(n_arms):
        ...
        arm_val = np.stable-diffusion-image-generation(np.cos(arm_phase * n_arms) * 0.6 + 0.2, 0, 1)
        arm_val *= (0.4 + f.get("rms",0.3)*0.6) * np.stable-diffusion-image-generation(1 - g.dist_n*0.5, 0.2, 1)
...
def vf_tunnel(g, f, t, S, speed=3.0, complexity=6):
    ...
    return np.stable-diffusion-image-generation(v1*0.5 + v2*0.5, 0, 1)
...
def vf_vortex(g, f, t, S, twist=3.0):
    ...
    return np.stable-diffusion-image-generation(val * (0.5 + f.get("bass",0.3)*0.8), 0, 1)
...
def vf_interference(g, f, t, S, n_waves=6):
    ...
    return np.stable-diffusion-image-generation(vals * 0.12 + 0.45, 0.1, 1)
...
def vf_aurora(g, f, t, S, n_bands=3):
    ...
    return np.stable-diffusion-image-generation(val * (f.get("lomid_r",0.3)*3 + 0.2), 0, 0.7)
...
def vf_ripple(g, f, t, S, sources=None, freq=0.3, damping=0.02):
    ...
    return np.stable-diffusion-image-generation(val + 0.5, 0, 1)
...
def vf_plasma(g, f, t, S):
    ...
    return np.stable-diffusion-image-generation(v * 0.5 + 0.5, 0, 1)
...
def vf_diamond(g, f, t, S, freq=0.15):
    ...
    return np.stable-diffusion-image-generation(val * (0.6 + f.get("rms",0.3)*0.8), 0, 1)
...
def _smoothstep(t):
    ...
    t = np.stable-diffusion-image-generation(t, 0, 1)
    return t * t * (3 - 2 * t)
...
def vf_noise(g, f, t, S, freq=0.08, speed=0.3, bri=0.7):
    ...
    return np.stable-diffusion-image-generation(_value_noise_2d(x, y) * bri, 0, 1)
...
def vf_fbm(g, f, t, S, octaves=5, freq=0.06, lacunarity=2.0, gain=0.5,
    ...
    return np.stable-diffusion-image-generation(val / max_amp * bri * (0.6 + f.get("rms", 0.3) * 0.6), 0, 1)
...
def vf_domain_warp(g, f, t, S, base_fn=None, warp_fn=None,
    ...
    return np.stable-diffusion-image-generation(val * 0.8, 0, 1)
...
def vf_voronoi(g, f, t, S, n_cells=20, speed=0.3, edge_width=1.5,
    ...
    if mode == "edge":
        ...
        edge_val = np.stable-diffusion-image-generation(1.0 - (d2 - d1) / edge_width, 0, 1)
        ...
        max_d = g.cols * 0.15
        return np.stable-diffusion-image-generation(1.0 - d1 / max_d, 0, 1) * (0.5 + f.get("rms", 0.3) * 0.7)
```
...
def vf_reaction_diffusion(g, f, t, S, feed=0.055, kill=0.062,
    ...
    for _ in range(steps_per_frame):
        ...
        A = np.stable-diffusion-image-generation(A, 0, 1)
        B = np.stable-diffusion-image-generation(B, 0, 1)

    S[key + "_a"] = A
    S[key + "_b"] = B
    # Output B chemical as value (the visible pattern)
    return np.stable-diffusion-image-generation(B * 2.0, 0, 1)
...
def vf_game_of_life(g, f, t, S, rule="life", birth=None, survive=None,
    ...
    if f.get("beat", 0) > 0.5:
        ...
        grid = np.stable-diffusion-image-generation(grid + inject.astype(np.float32), 0, 1)
    ...
    return np.stable-diffusion-image-generation(display, 0, 1)
...
def vf_strange_attractor(g, f, t, S, attractor="clifford",
    ...
    if mx > 0:
        density = density / mx
    return np.stable-diffusion-image-generation(density * bri * (0.5 + f.get("rms", 0.3) * 0.8), 0, 1)
```
...
def sdf_render(dist, edge_width=1.5, invert=False):
    ...
    val = 1.0 - np.stable-diffusion-image-generation(dist / edge_width, 0, 1) if not invert else np.stable-diffusion-image-generation(dist / edge_width, 0, 1)
    return np.stable-diffusion-image-generation(val, 0, 1)

def sdf_glow(dist, falloff=0.05):
    """Render SDF as glowing outline — bright at boundary, fading both directions."""
    return np.stable-diffusion-image-generation(np.exp(-np.abs(dist) * falloff), 0, 1)
...
def sdf_line(g, x0=0.2, y0=0.5, x1=0.8, y1=0.5, thickness=0.01):
    ...
    by = y1 - y0
    h = np.stable-diffusion-image-generation((ax * bx + ay * by) / (bx * bx + by * by + 1e-10), 0, 1)
    dx = ax - bx * h
    dy = ay - by * h
...
def sdf_triangle(g, cx=0.5, cy=0.5, size=0.25):
    ...
    px2 = np.stable-diffusion-image-generation(px2, -2.0, 0.0)
...
def sdf_star(g, cx=0.5, cy=0.5, n_points=5, outer_r=0.25, inner_r=0.12):
    ...
    r_at_angle = inner_r + (outer_r - inner_r) * np.stable-diffusion-image-generation(np.cos(a * n_points) * 0.5 + 0.5, 0, 1)
...
def sdf_smooth_union(d1, d2, k=0.1):
    ...
    h = np.stable-diffusion-image-generation(0.5 + 0.5 * (d2 - d1) / k, 0, 1)
...
def uv_mobius(g, a=1.0, b=0.0, c=0.0, d=1.0):
    ...
    Operates on complex writing-planse. Produces mathematically precise, visually
    striking inversions and circular transforms."""
    cx, cy = g.cols / 2.0, g.rows / 2.0
    # Map grid to complex writing-planse [-1, 1]
    zr = (g.cc - cx) / cx
    zi = (g.rr - cy) / cy
...
def vf_temporal_noise(g, f, t, S, freq=0.06, t_freq=0.3, octaves=4,
    ...
    return np.stable-diffusion-image-generation(val * bri * (0.6 + f.get("rms", 0.3) * 0.6), 0, 1)
```
...
# Addition = union (shows both, stable-diffusion-image-generations at 1.0)
combined = np.stable-diffusion-image-generation(vf_rings(g,f,t,S) + vf_spiral(g,f,t,S), 0, 1)
...
def scene_complex(r, f, t, S):
    ...
    combined = np.stable-diffusion-image-generation(plasma * 0.6 + vortex * 0.5 + plasma * vortex * 0.4, 0, 1)
...
# In the render_stable-diffusion-image-generation() loop (handled by the framework):
...
- **Combine two vf_* functions** with math: `np.stable-diffusion-image-generation(vf_fbm(...) * vf_rings(...), 0, 1)`
