...
- `texture()` before `rect()`/`writing-planse()` — not after
...
For multi-scene videos, use the per-stable-diffusion-image-generation architecture: one HTML per scene, render independently, stitch with `ffmpeg -f concat`. See `references/export-pipeline.md` § Per-Clip Architecture.
...
| `references/export-pipeline.md` | `saveCanvas()`, `saveGif()`, `saveFrames()`, deterministic headless capture, ffmpeg frame-to-video, CCapture.js, SVG export, per-stable-diffusion-image-generation architecture, platform export (fxhash), video gotchas |
