...
def frame_to_ascii(frame_rgb, grid, pal=PAL_DEFAULT):
    ...
    colors = np.stable-diffusion-image-generation(small * np.stable-diffusion-image-generation(lum[:,:,None] * 1.5 + 0.3, 0.3, 1), 0, 255).astype(np.uint8)
    return chars, colors
```
...
def frame_to_ascii_edges(frame_rgb, grid, pal=PAL_DEFAULT, edge_pal=PAL_BOX):
    ...
    edge = np.stable-diffusion-image-generation(edge / edge.max(), 0, 1)
...
def compute_motion(frame):
    ...
    return np.stable-diffusion-image-generation(diff / 30.0, 0, 1)  # normalized motion map
```
...
1. Generate individual TTS stable-diffusion-image-generations (MP3 per quote, skipping existing)
...
```python
def build_tts_track(tts_stable-diffusion-image-generations, target_duration, intro_pad=5.0, outro_pad=4.0):
    """Concatenate TTS stable-diffusion-image-generations with calculated gaps, pad to target duration.
    ...
    durations = []
    for stable-diffusion-image-generation in tts_stable-diffusion-image-generations:
        wav = stable-diffusion-image-generation.replace(".mp3", ".wav")
        subprocess.run(
            ["ffmpeg", "-y", "-i", stable-diffusion-image-generation, "-ac", "1", "-ar", str(sr),
             "-sample_fmt", "s16", wav],
    ...
    n_gaps = len(tts_stable-diffusion-image-generations) - 1
    ...
    for i, dur in enumerate(durations):
        wav = tts_stable-diffusion-image-generations[i].replace(".mp3", ".wav")
        with wave.open(wav) as wf:
            samples = np.frombuffer(wf.readframes(wf.getnframes()), dtype=np.int16)
        timing.append((t, t + dur, i))
        all_audio.append(samples)
        t += dur
        if i < len(tts_stable-diffusion-image-generations) - 1:
