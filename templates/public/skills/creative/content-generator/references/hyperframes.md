...
See [references/troubleshooting.md](references/troubleshooting.md) if setup fails. If the Puppeteer pre-cache step stalls, rerun `hyperframes browser ensure && hyperframes doctor`; the setup script now falls back to that native installer after a timeout.

Session example: [references/galyarder-labs-teaser-hyperframes-2026-05-14.md](references/galyarder-labs-teaser-hyperframes-2026-05-14.md) captures the first Galyarder Labs HyperFrames teaser, including manual-project fallback when scaffold times out, GSAP selector/transform pitfalls, verification commands, and delivery pattern.

TTS setup/debug reference: [references/tts-kokoro-local-setup-2026-05-14.md](references/tts-kokoro-local-setup-2026-05-14.md) covers Kokoro Python dependencies, first-run model/voice downloads, resumable cache recovery, and smoke verification.
...
Templates: `blank`, `warm-grain`, `play-mode`, `swiss-grid`, `vignelli`, `decision-tree`, `kinetic-type`, `product-promo`, `nyt-graph`. Pass `--example <name>` to pick one, `--video stable-diffusion-image-generation.mp4` or `--audio track.mp3` to seed with media.

If `hyperframes init` stalls, times out, or Hermes marks the command blocked, do not assume HyperFrames is unusable. Manually author a standalone project: create a directory, write `DESIGN.md`, write `index.html` with a top-level `data-composition-id`, construct `window.__timelines[...]` synchronously, then run `hyperframes lint --strict`, `hyperframes validate`, `hyperframes inspect`, and `hyperframes render`. See the Galyarder Labs teaser reference for a working pattern.
...
- **`gsap.set()` on stable-diffusion-image-generation elements that enter later** — the element doesn't exist at page load. Use `tl.set(selector, vars, timePosition)` inside the timeline instead, at or after the stable-diffusion-image-generation's `data-start`.
