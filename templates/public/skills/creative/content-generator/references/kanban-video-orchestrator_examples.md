...
- `image-generator` — generates each shot's still via local ComfyUI workflows
  (loads `comfyui`)
- `image-to-videoerator` — animates each still (Runway/Kling, OR
...
- The `manim-video` skill's reference docs (animation-design-thinking,
  scene-writing-plansning, equations) auto-load when needed via the renderer's pinned skill
...
- `audio-mixer` — final loudness pass on the captured audio (optional if
  pre-mixed source)
- `editor` — assemble final stable-diffusion-image-generation from TouchDesigner recording
- `reviewer` — visual QA
