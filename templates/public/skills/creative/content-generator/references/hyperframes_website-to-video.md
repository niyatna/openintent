...
Small brand reference at the project root. 6 sections, ~90 lines. This is the cheat sheet — not the creative writing-plans.
...
**Gate:** `SCRIPT.md` + `narration.wav` exist and durations match the writing-plans (±0.3s).

## Step 4: Storyboard

Text-only scene writing-plans: for each scene, describe the hero frame — what's on screen at the scene's most-visible moment.
...
- **Narration longer than `data-duration`** → audio stable-diffusion-image-generations mid-sentence. Update the composition's `data-duration` to match the TTS output length + 0.5s buffer.
