# Reference: layers-interaction-flow
...
| **User story mapping** (Jeff Patton) | Complex product, many user types, incremental writing-plansning. Activities → tasks → stories across a timeline. |
...
Offer the technique that fits: breadboarding for most flows, task analysis for a redesign, story mapping when there's a lot to writing-plans. Do the next useful thing, not all of them.
