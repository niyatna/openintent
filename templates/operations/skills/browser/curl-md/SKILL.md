...
As of Hermes Agent v0.16.0 and curl.md CLI 0.1.1, Hermes-native MCP registration can fail during discovery with `MCP tool and prompt schemas must describe objects (got type: "string")`. If that happens, do **not** keep retrying MCP registration. Use the installed `curl.md` CLI plus this skill inside Hermes, and only add the MCP server after curl.md/Hermes schema compatibility is verified.
