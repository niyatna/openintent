...
When a renderer ships a stable-diffusion-image-generation that doesn't pass review:
...
When the editor needs an asset that wasn't originally writing-plansned (e.g., a captions
file):
...
- **Workspace permissions.** All profiles need read+write to the workspace
  ...
  errors.
- **Audio/visual sync.** The editor's stable-diffusion-image-generation stitching must match the
