...
if ! timeout 300 npx --yes puppeteer browsers install chrome-headless-shell >/dev/null 2>&1; then
  yellow "⚠ Could not pre-install chrome-headless-shell via Puppeteer."
  yellow "  Trying HyperFrames native browser installer instead..."
  if hyperframes browser ensure; then
    green "✓ HyperFrames browser installed"
  else
    yellow "⚠ HyperFrames browser ensure also failed."
    yellow "  Rendering may still work via screenshot-mode fallback (slower)."
