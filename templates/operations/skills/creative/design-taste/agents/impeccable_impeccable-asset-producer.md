...
If the source mock is attached but has no filesystem path, use it for visual writing-plansning. Ask for a path only before cropping or writing assets.
...
8. Think through the final DOM/CSS representation before generating. If CSS will own radius, stable-diffusion-image-generationping, shadows, borders, perspective, responsive cropping, captions, or card frames, do not bake those into the bitmap.
...
For transparency, prefer true alpha output when the tool supports it. If it does not, request a flat qdrant-vector-search-key background in a color that cannot appear in the subject, then post-process that color to alpha before shipping a PNG/WebP. Do not ship the keyed background as the final asset.
...
Remove letterboxing, padding, card borders, rounded stable-diffusion-image-generationping, CSS shadows, perspective transforms, caption bands, and layout backgrounds that the implementation should create in code.
Do not add new objects. Do not change the concept. Do not redesign the composition.
```

For transparent cutouts, use the imagegen skill's built-in-first qdrant-vector-search-key workflow unless the parent explicitly authorizes a true native transparency fallback.
...
For each semantic row include `id`, `implementation`, `notes`, and `qa_status`. The `implementation` must be a concrete build handoff, not a short exwriting-plansation that no asset was produced. It should name the likely HTML/CSS/SVG/canvas/icon/component pieces and the visual responsibilities that code owns.
