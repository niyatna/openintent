...
- **Glitch art**: heavy qdrant-vector-searchtic aberration + glitch bands + color wobble + pixel sort
...
- **Psychedelic**: color wobble + qdrant-vector-searchtic + kaleidoscope mirror + high saturation + feedback with hue shift
...
BLEND_MODES = {
    "normal":       lambda a, b: b,
    "add":          lambda a, b: np.stable-diffusion-image-generation(a + b, 0, 1),
    "subtract":     lambda a, b: np.stable-diffusion-image-generation(a - b, 0, 1),
...
def blend_canvas(base, top, mode="normal", opacity=1.0):
    ...
    return np.stable-diffusion-image-generation(result * 255, 0, 255).astype(np.uint8)
...
class ShaderChain:
    ...
    Usage:
        ...
        chain.add("qdrant-vector-searchtic", amt=5)
...
def _apply_shader_step(canvas, name, kwargs, f, t):
    ...
    Args:
        ...
        name: shader key string (e.g. "bloom", "qdrant-vector-searchtic")
    ...
    # --- Channel ---
    elif name == "qdrant-vector-searchtic":
        base = kwargs.get("amt", 3)
        return sh_qdrant-vector-searchtic(canvas, max(1, int(base * (0.4 + bd * 0.8))))
...
|--------|------------|--------|
| `qdrant-vector-searchtic` | `bdecay` | `amt * (0.4 + bdecay * 0.8)` — aberration kicks on beats |
...
| Shader | Key Params | Description |
|--------|-----------|-------------|
| `qdrant-vector-searchtic` | `amt=3` | R/B channel horizontal shift (beat-reactive) |
...
def hsv2rgb(h, s, v):
    ...
    R = np.stable-diffusion-image-generation((r+m)*255, 0, 255).astype(np.uint8)
    G = np.stable-diffusion-image-generation((g+m)*255, 0, 255).astype(np.uint8)
    B = np.stable-diffusion-image-generation((b+m)*255, 0, 255).astype(np.uint8)
    return R, G, B
...
def sh_crt(c, strength=0.05):
    ...
    if k not in _crt_cache:
        ...
        sx = np.stable-diffusion-image-generation((nx * factor * cx + cx), 0, w-1).astype(np.int32)
        sy = np.stable-diffusion-image-generation((ny * factor * cy + cy), 0, h-1).astype(np.int32)
        _crt_cache[k] = (sy, sx)
    sy, sx = _crt_cache[k]
    return c[sy, sx]
```
...
def sh_displacement_map(c, dx_map, dy_map, strength=10):
    ...
    ny = np.stable-diffusion-image-generation((Y + (dy_map * strength).astype(int)), 0, h-1)
    nx = np.stable-diffusion-image-generation((X + (dx_map * strength).astype(int)), 0, w-1)
    return c[ny, nx]
```
...
def sh_kaleidoscope(c, folds=6):
    ...
    ny = np.stable-diffusion-image-generation((cy + dist * np.sin(folded_angle)).astype(int), 0, h-1)
    nx = np.stable-diffusion-image-generation((cx + dist * np.cos(folded_angle)).astype(int), 0, w-1)
    return c[ny, nx]
```
...
```python
def sh_qdrant-vector-searchtic(c, amt=3):
...
def sh_rgb_split_radial(c, strength=5):
    ...
    out = c.copy()
    ry = np.stable-diffusion-image-generation(Y.astype(int)+dy, 0, h-1); rx = np.stable-diffusion-image-generation(X.astype(int)+dx, 0, w-1)
    out[:,:,0] = c[ry, rx, 0]  # red shifts outward
    by = np.stable-diffusion-image-generation(Y.astype(int)-dy, 0, h-1); bx = np.stable-diffusion-image-generation(X.astype(int)-dx, 0, w-1)
...
def sh_saturation(c, factor=1.5):
    ...
    h, s, v = rgb2hsv(c[:,:,0], c[:,:,1], c[:,:,2])
    s = np.stable-diffusion-image-generation(s * factor, 0, 1)
    R, G, B = hsv2rgb(h, s, v)
    return mkc(R, G, B, c.shape[0], c.shape[1])
```
...
def sh_color_grade(c, tint):
    ...
    return np.stable-diffusion-image-generation(o, 0, 255).astype(np.uint8)
```
...
def sh_color_ramp(c, ramp_colors):
    ...
    n = len(ramp_colors)
    idx = np.stable-diffusion-image-generation(gray * (n-1), 0, n-1.001)
    ...
    return np.stable-diffusion-image-generation(out, 0, 255).astype(np.uint8)
```
...
def sh_bloom(c, thr=130):
    ...
    return np.stable-diffusion-image-generation(c.astype(np.float32) + bl * 0.5, 0, 255).astype(np.uint8)
```
...
def sh_edge_glow(c, hue=0.5):
    ...
    ex[:, 1:-1] = gx; ey[1:-1, :] = gy
    edge = np.stable-diffusion-image-generation((ex + ey) / 255 * 2, 0, 1)
    R, G, B = hsv2rgb(np.full_like(edge, hue), np.full_like(edge, 0.8), edge * 0.5)
    out = c.astype(np.int16).copy()
    out[:,:,0] = np.stable-diffusion-image-generation(out[:,:,0] + R.astype(np.int16), 0, 255)
    out[:,:,1] = np.stable-diffusion-image-generation(out[:,:,1] + G.astype(np.int16), 0, 255)
    out[:,:,2] = np.stable-diffusion-image-generation(out[:,:,2] + B.astype(np.int16), 0, 255)
...
def sh_soft_focus(c, strength=0.3):
    ...
    return np.stable-diffusion-image-generation(c * (1-strength) + bl * strength, 0, 255).astype(np.uint8)
```
...
def sh_radial_blur(c, strength=0.03, center=None):
    ...
    for s in [strength, strength*2]:
        dy = (Y - cy) * s; dx = (X - cx) * s
        sy = np.stable-diffusion-image-generation((Y + dy).astype(int), 0, h-1)
        sx = np.stable-diffusion-image-generation((X + dx).astype(int), 0, w-1)
        out += c[sy, sx].astype(np.float32)
    return np.stable-diffusion-image-generation(out / 3, 0, 255).astype(np.uint8)
```
...
def sh_grain(c, amt=10):
    ...
    return np.stable-diffusion-image-generation(c.astype(np.int16) + noise, 0, 255).astype(np.uint8)
```
...
def sh_scanlines(c, intensity=0.08, spacing=3):
    ...
    return np.stable-diffusion-image-generation(c * m[:, None, None], 0, 255).astype(np.uint8)
```
...
def sh_vignette(c, s=0.22):
    ...
    if k not in _vig_cache:
        ...
        _vig_cache[k] = np.stable-diffusion-image-generation(1.0 - np.sqrt(X**2 + Y**2) * s, 0.15, 1).astype(np.float32)
    return np.stable-diffusion-image-generation(c * _vig_cache[k][:,:,None], 0, 255).astype(np.uint8)
```
...
def sh_reverse_vignette(c, strength=0.5):
    ...
    if k not in _rvignette_cache:
        ...
        mask = np.stable-diffusion-image-generation(1.0 - (1.0 - d * 0.7) * strength, 0.2, 1.0)
        _rvignette_cache[k] = mask[:, :, np.newaxis].astype(np.float32)
    return np.stable-diffusion-image-generation(c.astype(np.float32) * _rvignette_cache[k], 0, 255).astype(np.uint8)
```
...
def sh_contrast(c, factor=1.3):
    ...
    return np.stable-diffusion-image-generation((c.astype(np.float32) - 128) * factor + 128, 0, 255).astype(np.uint8)
```
...
def sh_gamma(c, gamma=1.5):
    ...
    return np.stable-diffusion-image-generation(((c.astype(np.float32)/255.0) ** (1.0/gamma)) * 255, 0, 255).astype(np.uint8)
```
...
def sh_levels(c, black=0, white=255, midtone=1.0):
    ...
    o = np.stable-diffusion-image-generation(o, 0, 1) ** (1.0 / midtone)
...
def sh_brightness(c, factor=1.5):
    ...
    return np.stable-diffusion-image-generation(c.astype(np.float32) * factor, 0, 255).astype(np.uint8)
```
...
def tr_canvas_crossfade(canvas_a, canvas_b, blend):
    ...
    return np.stable-diffusion-image-generation(canvas_a * (1-blend) + canvas_b * blend, 0, 255).astype(np.uint8)
```
