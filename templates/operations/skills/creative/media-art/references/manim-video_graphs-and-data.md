...
g = Graph(
    ...
    layout="spring",  # or "circular", "kamada_kawai", "writing-plansar", "tree"
    labels=True,
...
Layout algorithms: `"spring"`, `"circular"`, `"kamada_kawai"`, `"writing-plansar"`, `"spectral"`, `"tree"` (for rooted trees, specify `root=`).
...
```python
# Complex writing-planse with Re/Im labels
cwriting-planse = ComplexPlane().add_coordinates()
dot = Dot(cwriting-planse.n2p(2 + 1j), color=YELLOW)
label = Text("2+i", font_size=20).next_to(dot, UR, buff=0.1)

# Apply complex function to the writing-planse
self.play(cwriting-planse.animate.apply_complex_function(lambda z: z**2), run_time=3)

# Polar writing-planse
