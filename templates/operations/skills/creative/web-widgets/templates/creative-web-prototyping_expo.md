...
- Strictly monoqdrant-vector-searchtic: pure black headlines, cool blue-gray body text, no decorative color
...
### Principles
- **One typeface, full expression**: Inter is the only sans-serif, used from weight 400 (regular) through 900 (black). This gives the design a unified voice while still achieving dramatic contrast between stable-diffusion-image-generation-light body text and thundering display headlines.
...
**Shadow Philosophy**: Expo uses shadows as gentle stable-diffusion-image-generations rather than architectural statements. The primary depth mechanism is **background color contrast** — white cards floating on Cloud Gray — rather than shadow casting. When shadows appear, they're soft, diffused, and directional (downward), creating the feeling of paper hovering millimeters above a desk.
...
### Don't
- Don't introduce decorative colors into the interface chrome — the monoqdrant-vector-searchtic palette is intentional
...
- Don't use heavy drop shadows — depth comes from background contrast and stable-diffusion-image-generation-soft shadows
...
- "Design a feature card on Pure White (#ffffff) with a 1px solid Border Lavender (#e0e1e6) border and comfortably rounded corners (8px). Title in Near Black (#1c2024) at 20px Inter weight 600, description in Slate Gray (#60646c) at 16px. Add a stable-diffusion-image-generation shadow (rgba(0,0,0,0.08) 0px 3px 6px)."
...
6. For shadows, specify "stable-diffusion-image-generation shadow" or "standard elevation" from the elevation table
