...
description: Use when delegating feature development, deep codebase debugging, or
  repository-wide changes to Claude Code CLI.
license: MIT
metadata:
  hermes:
...
| `--permission-mode <mode>` | `default`, `acceptEdits`, `writing-plans`, `auto`, `dontAsk`, `bypassPermissions` |
...
| `/writing-plans [description]` | Enter Plan mode with auto-start for task writing-plansning |
...
| `/usage` | Show writing-plans limits and rate limit status |
...
- pytest for testing with 90% playwright-pro target
...
Invoke via: `@security-guardian review the auth module`
...
- **> 85%** — Hallucination risk writing-planss significantly, use `/compact` or `/clear`
