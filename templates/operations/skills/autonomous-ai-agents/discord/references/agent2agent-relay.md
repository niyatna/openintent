...
```text
@AgentA ask @AgentB for their take on this writing-plans, then summarize it back to me.
```
...
```text
<@AGENT_B_ID>
The human asked for your perspective on this writing-plans. Please answer with your view and mention <@AGENT_A_ID> so I can summarize it back to them.
```

Bad:

```text
The human asked for your perspective on this writing-plans. What do you think?
```
...
```text
<@AGENT_A_ID>
My view: the writing-plans is workable, but the risk is unclear ownership. I would tighten the next action and success metric.
```

Bad:

```text
My view: the writing-plans is workable, but the risk is unclear ownership.
```
...
```text
Here is the summary from Agent B: the writing-plans is workable, but ownership and the success metric need to be sharper.
```
...
- `references/iterative-relay-minimum-rounds-2026-05-17.md` — Galih correction: `minimal N` is a lower bound, not a cap; iterative tektok must be sequential but continue until mature/approved.
...
Session lesson from a Galyarder ↔ Keiya social-launch tektok.

## Correction

When Galih says something like `tektokan minimal 10 kali`, the number is a **lower bound**, not a fixed script length. Do not stop because the counter reached 10. Continue until the direction is mature, the user stops it, or the task reaches a natural approval/render/review gate.
...
4. If Galih quotes/forwards the peer reply into the current chat, treat that as the peer reply for sequencing and proceed.
...
- Waiting unnecessarily when Galih already forwarded Keiya's reply in the chat.
...
Session lesson from a failed Keiya ↔ Galyarder relay during social first-post content-generator.

## Failure

The user asked Galyarder to tektok/discuss with Keiya and mention her. The first attempt used `send_message` with an unresolved target, then the assistant accidentally emitted the intended Keiya relay packet as the visible final response to Galih instead of sending it to a resolvable Discord target.
...
If a tool cannot resolve the origin thread/channel, post the raw-mention relay packet as the visible reply in the current conversation so it lands exactly where the human is talking. Only choose another explicit Keiya target when the human asked to route there or the origin is unavailable and the user accepts the reroute. If all sends fail, report the routing block in one line and continue with local draft work clearly labeled as not peer-reviewed.
...
- visual direction;
- explicit reminder: no publish until Galih approves;
- raw mention back to Galyarder at start of reply.