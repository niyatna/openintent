...
For a channel where Keiya, Galyarder, or other Hermes agents need to call each other:
...
Use raw Discord mention objects such as `<@BOT_USER_ID>` for deterministic bot pings; display-name text like `@Galyarder` may not resolve from gateway-authored messages. See `references/discord-controlled-relay-protocol.md` for session details and verification pattern.

### Agent-facing relay behavior

When the user asks you to consult another Discord agent from a Discord thread/channel, **the origin conversation is the route**. If the human says “mention Keiya/Galyarder” while already chatting in a channel/thread, do not use `send_message` or manually pick a different target. Reply in the current/origin conversation with the raw bot mention at the start; the gateway/platform delivery layer will post the final response back to the same place. Use `send_message` only when the human explicitly asks to send/relay somewhere else, or when you are outside a Discord-origin turn and must choose a destination.
...
```text
<@TARGET_BOT_ID> konteks: Galih minta satu sudut pandang dari kamu.
pertanyaan: apa hal penting yang perlu aku sampaikan balik ke Galih?
...
Human-facing summary after the target replies should not contain the target bot mention unless Galih explicitly asks you to ping it again.
...
For a simple human-authorized agent-to-agent relay (for example `mention Keiya, saling sapa`), skill loading is internal routing discipline, not the visible deliverable. Do not answer Galih with raw relay packets, tool logs, or a lecture about which skill was loaded unless he explicitly asks for an audit trail. Execute the controlled relay once, then report tersely: sent / blocked / peer replied.
...
Do not infer the voice/register from the Discord server/channel label alone. Ground it from the active profile/speaker. Keiya/default uses Indonesian `aku-kamu` even when the channel is named Galyarder Labs or the topic is technical/runtime. Galyarder/blade mode uses `lu-gua` with `gua` (never `gua`, wait, let me check the instruction: "Galyarder/blade mode uses lu-gua with gua (never gue)"). If Keiya relays or quotes Galyarder, lu-gua may appear only as quoted/relayed Galyarder text, not Keiya's own voice.
...
- `references/discord-controlled-relay-protocol.md` — live Keiya ↔ Galyarder mention test, raw mention lesson, and controlled relay protocol.
