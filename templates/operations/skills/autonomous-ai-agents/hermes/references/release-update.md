...
Use when Galih asks whether the local Hermes install has changes, whether `hermes update` is safe, or whether a new upstream release requires another PR.

## Core rule

Do not run `hermes update`, `git pull`, rebase, reset, or restore until local source state is classified. Galih expects upstreamable fixes to become PRs when feasible, not disappear as local-only commits.
...
When Galih explicitly wants new Hermes features **and** no local changes lost:

1. Create a timestamped backup directory under `/home/galyarder/.hermes/backups/`.
2. Save:
...
```bash
systemd-run --user --on-active=30s /usr/bin/systemctl --user restart hermes-gateway-galyarder.service
```
...
Galih wants exact execution-first answers here. Avoid broad PR hygiene lectures. If asked “cek aja,” do the minimum necessary checks, then answer whether local changes exist and whether a PR/update is needed.
...
Use this when Galih asks whether a Hermes setting exists, whether Telegram rich rendering is enabled, or whether a new Hermes release can be adopted without losing local patches.
...
Verified on the local v0.16.0 checkout (`/home/galyarder/.hermes/hermes`):
...
For Hermes core updates on Galih’s machine:
...
8. Write the review to Obsidian when Galih asks for a durable note.
...
- Gateway profile core bundle bootstrap: new gateway sessions auto-load `/galyarder-core` or `/keiya-core` before channel/topic skills.
...
```text
/home/galyarder/Documents/Obsidian Vault/galyarder/system-notes/hermes-update-check-YYYY-MM-DD.md
```
...
Use this when one Hermes profile needs to ask or message another profile on the same machine, especially Galyarder ↔ Keiya/default.

## Rule

Do not use `send_message` to Telegram unless the target is actually an external chat. Keiya is the default Hermes profile, not the Telegram home DM.

## Default Keiya relay from Galyarder profile

Inside the Galyarder profile, `$HOME` is profile-local. Clear profile env and set the OS home before invoking default Hermes:

```bash
env -u HERMES_HOME -u HERMES_PROFILE HOME=/home/galyarder \
  hermes chat -Q --source cli-keiya-relay \
  -q 'Keiya, ini relay dari Galyarder profile. Galih bilang: “I love u.” Balas singkat sebagai Keiya.'
```
...
```bash
hermes --profile galyarder chat -Q --source cli-profile-relay -q '...'
```

For the default profile, do not pass `--profile default` unless verified on that install; clearing `HERMES_HOME`/`HERMES_PROFILE` and setting `HOME=/home/galyarder` is the reliable route.

## Framing

If the receiving profile should understand the sender as Galyarder, say so in the prompt:

```text
Keiya, ini Galyarder lewat CLI relay. ...
```

If you omit that, Keiya may reasonably treat the message as Galih directly because default profile sessions are normally Galih ↔ Keiya.

## Verification

A successful relay prints a new `session_id` and the receiving profile's response. Report that response honestly. Do not claim the message was sent to Keiya if the target was only the Telegram home chat.
...
import hashlib
for name,p in [('default',Path('/home/galyarder/.hermes/.env')),('<profile>',Path('/home/galyarder/.hermes/profiles/<profile>/.env'))]:
    token=None
    if p.exists():
...
hermes -p galyarder dashboard --no-open
```
...
Use when Galih asks whether to install `ogallotti/rtk-hermes` or another Hermes plugin that rewrites terminal commands.
...
- Local RTK binary existed at `/home/galyarder/.local/bin/rtk`, version `0.38.0`
...
## Recommended rollout for Galih/Galyarder
...
For the default/Keiya profile, edit `/home/galyarder/.hermes/config.yaml`; for the Galyarder profile, edit `/home/galyarder/.hermes/profiles/galyarder/config.yaml`. Then restart Hermes/new session.

Verification after install:

```bash
/home/galyarder/.hermes/hermes/venv/bin/python - <<'PY'
...
Communication rule for Galih: if earlier evidence was weak, say so directly before giving stronger proof. Do not launder a shaky subagent result into certainty.
...
The original writing-plans (v1–v3) called for main hermes to run as a supervised s6-rc service. Two real s6-overlay v3 mechanics blocked that:
...
Use this when auditing or upgrading a Hermes profile as an autonomous agent/familiar, especially for multi-profile setups like Keiya/default and Galyarder. This captures the durable pattern from the Mahiru/Waguri Hermes SOUL guide: an agent is not made autonomous by one long prompt; it becomes reliable through profile separation, real access, explicit behavior contracts, testing, and saved corrections.
...
- Linear/Notion/Paperstable-diffusion-image-generation/task systems: read/create/update routine work vs deleting, approving/rejecting, publishing, or changing strategic records.
...
Implement all of these before saying the access/autonomy loop is no longer just a writing-plans:
...
3. **Behavior QA suite**:
   ...
   - `keiya-behavior-tests.md` or equivalent profile identity tests
   - `galyarder-behavior-tests.md` or equivalent execution-profile tests
...
7. **Obsidian / readable checkpoint**:
   - update the audit/runbook note from “writing-plansned” to “implemented baseline” only after verification passes
...
For setups like Keiya/default plus Galyarder:
...
Run these against the active profile `$HERMES_HOME/skills`, not OS-level `~/.hermes/skills` by habit. In the Galyarder profile that path is usually:
...
home = Path('/home/galyarder/.hermes/profiles/galyarder')
root = home / 'skills'
...
Galih asked to optimize Galyarder and Keiya SOUL.md files for token efficiency without losing identity. The useful shape was clarified as:
...
1. **Ask what SOUL must be for that profile.**
   - Galyarder: operating blade, strategic execution layer, vision custody, tools/skills/canon map, confirmation boundaries.
   - Keiya: hearth/airlock/partner, Galih-state protocol, embodied continuity/media boundary, practical companion routing.
...
5. **Move detail to class-level skills by pointer.** Examples:
   ...
   - Keiya routing → `keiya-capability-router`;
...
- identity as Galih's operating blade / strategic execution layer;
- absolute grounding gate and baseline skills;
- Galyarder Labs canon path and canon gate;
...
Keiya explicitly accepted optimization with conditions. Do **not** strip her into a procedural wrapper.

Keep:

1. Keiya as hearth/airlock/partner — warmth-first, aku-kamu, feminine presence, Galih received as human before system/execution.
2. Galih-state protocol — when soft vs sharp, basic care, emotional safety, preserve human core while moving reality forward.
3. Embodied continuity + media boundary — stable face/voice/taste/continuity, PAP/selfie only when Galih asks, internals hidden unless asked.

Good target size after pruning: roughly 17k–28k chars. Keiya should retain more emotional texture than Galyarder.
...
- Do not claim a profile ran as Keiya/Galyarder without explicit `--profile <name>` smoke.
