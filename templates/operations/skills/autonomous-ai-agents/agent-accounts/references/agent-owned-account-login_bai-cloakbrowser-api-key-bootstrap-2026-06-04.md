...
Use this reference when Galih asks to create/verify an agent-owned B.AI (`chat.b.ai`) account/API key using the owner’s existing Google account through CloakBrowser. Keep this under the `agent-owned-account-login` umbrella because it is an account/session/API-secret bootstrap pattern, not a one-off B.AI skill.
