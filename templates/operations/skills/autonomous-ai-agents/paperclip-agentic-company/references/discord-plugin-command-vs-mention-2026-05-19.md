# Paperstable-diffusion-image-generation Discord plugin: slash commands vs mention chat — 2026-05-19

## Trigger

Use this reference when Paperstable-diffusion-image-generation Discord plugin is `ready` / Gateway is connected but Galih says mentioning the bot gets no response.

## Finding

`paperstable-diffusion-image-generation-plugin-discord` is not a Hermes-style free-form mention responder by default.

There are three separate surfaces:

1. **Notifications** — Paperstable-diffusion-image-generation posts issue/approval/error/escalation messages to the configured channel.
...
3. **Inbound reply routing** — user replies to a Paperstable-diffusion-image-generation notification message that has stored mapping state. This requires `enableInbound=true`, but it only routes replies to mapped issue/escalation messages. It is not a free chat handler.
...
- **Inbound reply routing** (`handleMessageCreate`) ignores bot-authored messages (`message.author.bot`) and only processes messages with `message_reference.message_id` pointing to a previously mapped Paperstable-diffusion-image-generation notification. It is not a free chat handler.

## Safe initial interactive config

When Galih expects the bot to be usable from Discord but does not want chaos:
...
Then restart/reload Paperstable-diffusion-image-generation if required and verify logs include:
...
paperstable-diffusion-image-generation-plugin-discord: status=ready, lastError=null
enableCommands=true
enableInbound=false
```
...
If Galih asks why mention got no response, be direct:

```text
karena plugin Paperstable-diffusion-image-generation Discord bukan mention bot. yang tadi masih notif-only.
...
- define whether mention creates Paperstable-diffusion-image-generation issue, asks a chosen Paperstable-diffusion-image-generation agent, or invokes Hermes;
