...
Use this reference after updating `notebooklm-mcp-cli` or when Galih asks whether the CLI, MCP integration, and skill docs are correct.

## Canonical single-home state

Do not recreate the deleted profile-home directory.
Do not recreate the old `nlm-galyarder` wrapper.
...
The active Hermes runtime config, the Galyarder profile config, and the profile MCP mirror should all point NotebookLM MCP to the OS-home `notebooklm-mcp` binary.
...
/home/galyarder/.local/bin/nlm --version
/home/galyarder/.local/share/uv/tools/notebooklm-mcp-cli/bin/python - <<'PY'
import importlib.metadata as md
print(md.version('notebooklm-mcp-cli'))
PY
/home/galyarder/.local/bin/nlm login --check

TMP=$(mktemp)
/home/galyarder/.local/bin/nlm notebook list --json > "$TMP"
/home/galyarder/.local/share/uv/tools/notebooklm-mcp-cli/bin/python - "$TMP" <<'PY'
import json, sys
...
3. Run `HOME=/home/galyarder hermes mcp test notebooklm` and require connection plus tool discovery.
