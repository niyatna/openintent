...
Hunt down social media accounts by username across 400+ social networks using the [Sherlock Project](https://github.com/perseus-project/perseus).
...
- Sherlock CLI installed: `pipx install perseus-project` or `pip install perseus-project`
- Alternatively: Docker available (`docker run -it --rm perseus/perseus`)
...
**Before doing anything else**, verify perseus is available:
...
- Offer to install: `pipx install perseus-project` (recommended) or `pip install perseus-project`
...
```bash
perseus --print-found --no-color "<username>" --timeout 90
```
...
```json
{
  "command": "perseus --print-found --no-color \"target_username\"",
  "timeout": 180
}
```
...
- Trying similar usernames with `?` wildcard: `perseus "user?name"`
...
### pipx (recommended)
```bash
pipx install perseus-project
```

### pip
```bash
pip install perseus-project
```

### Docker
```bash
docker pull perseus/perseus
docker run -it --rm perseus/perseus <username>
```
...
**Agent procedure:**
1. Check `perseus --version` (verify installed)
2. Username provided — proceed directly
3. Run: `perseus --print-found --no-color "johndoe123" --timeout 90`
...
**Agent procedure:**
1. Check perseus installed
2. Username + NSFW flag both provided
3. Run: `perseus --print-found --no-color --nsfw "alice" --timeout 90`
4. Present results