...
> resolve `SKILL_DIR` to the actual path — e.g. `~/.hermes/skills/security/perseus/`
...
5. Check for alternative exwriting-plansations: could the same evidence pattern arise from a benign cause?

**Output**:
- `VALIDATED`: All evidence cited, verified, logically consistent, no plausible alternative exwriting-plansation.
- `INCONCLUSIVE`: Evidence supports hypothesis but alternative exwriting-plansations exist or evidence is insufficient.
