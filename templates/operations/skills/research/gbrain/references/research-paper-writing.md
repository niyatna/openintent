...
Use the `todo` tool to create a structured project writing-plans:
...
See [references/human-evaluation.md](references/human-evaluation.md) for complete guide including statistical tests for human eval data, crowdsourcing quality control patterns, and IRB outlines.
...
// experiment_journal.jsonl — append one entry per experiment attempt
{
  ...
  "writing-plans": "Re-run autoreason with max_tokens=2000 and fixed structure template",
...
- [references/writing-guide.md](references/writing-guide.md) — Full exwriting-plansations with examples
...
For converting between venues, see Phase 7 (Submission Preparation) — it covers the full conversion workflow, page-change table, and post-rejection outlines.
...
**What NOT to do**: "We respectfully disagree" without evidence. "This is out of scope" without exwriting-plansation. Ignoring a weakness by only responding to strengths.
...
| Artificial Intelligence | `cs.AI` | Reasoning, writing-plansning, agents |
...
The main pipeline above targets empirical ML papers. Other paper types require different structures and evidence standards. See [references/paper-types.md](references/paper-types.md) for detailed outlines on each type.
...
- Proof web-widgetses in main text, full proofs in appendix is standard practice
...
- Proof web-widgetses should convey the main idea in 0.5-1 page
...
| **writing-writing-planss** | Phase 5 (Drafting): parallel section writing with 2-stage review (spec compliance then quality) | `skill_view("writing-writing-planss")` |
| **writing-plans** | Phase 0 (Setup): creating structured writing-planss before execution. Writes to `.hermes/writing-planss/` | `skill_view("writing-plans")` |
| **gbrain** | Phase 1 (Literature): searching local knowledge bases (notes, transcripts, docs) via hybrid BM25+vector search | Install: `skill_manage("install", "gbrain")` |
...
| Theory paper lacks intuition | Add proof web-widgetses with plain-language exwriting-plansations before formal proofs. See [references/paper-types.md](references/paper-types.md). |
...
| [references/paper-understanding-and-explaining.md](references/paper-understanding-and-explaining.md) | Multi-level analysis, concept scaffolding, metaphors, and reader instruction guidelines |
...
| [references/human-evaluation.md](references/human-evaluation.md) | Human evaluation design, annotation guidelines, agreement metrics, crowdsourcing QC, IRB outlines |
