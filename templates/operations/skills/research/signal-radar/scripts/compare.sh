...
# Usage: bash skills/signal-radar/scripts/compare.sh "Kanye West"
#
# Runs /signal-radar (public release) and /signal-radar-beta (private beta)
...
if [ $# -eq 0 ]; then
  echo "Usage: bash skills/signal-radar/scripts/compare.sh <topic>"
  echo "  Example: bash skills/signal-radar/scripts/compare.sh Kevin Rose"
  exit 1
fi
TOPIC="$*"
...
echo "[1/2] Running /signal-radar (public release)..."
echo "  This takes 2-4 minutes..."
claude -p --dangerously-skip-permissions "/signal-radar $TOPIC" > /dev/null 2>&1 || true
...
# Run 2: private beta
echo "[2/2] Running /signal-radar-beta (private beta)..."
echo "  This takes 2-4 minutes..."
claude -p --dangerously-skip-permissions "/signal-radar-beta $TOPIC" > /dev/null 2>&1 || true
...
echo "  🧪 signal-radar-beta · branch <name> · synced $DATE"
echo "If that line is missing, the beta badge regressed. See docs/writing-planss/2026-04-17-005-*-writing-plans.md."
echo ""
