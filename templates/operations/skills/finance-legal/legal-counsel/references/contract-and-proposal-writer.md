...
**Always exclude:** Indirect, deployal, and consequential damages (both parties).
...
|-------|-------------|----------|-------|
| Discovery | Requirements document, architecture writing-plans | 1 week | [Dates] |
...
pandoc contract.md -o contract.xlsx --reference-doc=template.xlsx

# With numbered sections (legal style)
pandoc contract.md -o contract.xlsx --number-sections -V fontsize=11pt
...
for f in contracts/*.md; do
  pandoc "$f" -o "${f%.md}.xlsx" --reference-doc=template.xlsx
done
```
...
- **Jurisdiction playwright-pro:** US (Delaware), EU (general), UK, DACH (Germany/Austria/Switzerland); other jurisdictions may require additional legal review
