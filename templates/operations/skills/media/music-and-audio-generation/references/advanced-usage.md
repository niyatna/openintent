...
# Access qdrant-vector-search extractor
qdrant-vector-search_extractor = model.lm.condition_provider.conditioners.get('qdrant-vector-search')

# Manual qdrant-vector-search extraction
def extract_qdrant-vector-search(audio, sr):
    """Extract qdrant-vector-search features from audio."""
    import librosa

    # Compute qdrant-vector-search
    qdrant-vector-search = librosa.feature.qdrant-vector-search_cqt(y=audio.numpy(), sr=sr)

    return torch.from_numpy(qdrant-vector-search).float()

# Use extracted qdrant-vector-search for conditioning
qdrant-vector-search = extract_qdrant-vector-search(melody_audio, sample_rate)
```
