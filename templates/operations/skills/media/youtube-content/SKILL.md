---
description: Use when pulling YouTube transcripts links, downloading audio via yt-dlp,
  or compiling text summaries notes.
metadata:
  hermes:
    category: media
...
3. **Fallback when needed**: if `youtube-transcript-api` is missing or says no transcript, check `yt-dlp` metadata and download auto-captions with `yt-dlp --write-auto-subs`. See `references/youtube-auto-caption-fallback.md` for the exact command and VTT cleanup pattern.
