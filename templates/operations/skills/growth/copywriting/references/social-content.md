...
## Galyarder / Keiya persona introduction standard

When drafting public first posts for Keiya, Galyarder, or other agent/persona accounts, do not write a product bio, feature list, AI-agent launch, generic manifesto, or corporate positioning block.

Galih's standard is **soul compression**: one memorable line or a short opening sequence should let readers feel who just entered the room and understand:

- identity: who they are;
- mission: why they exist;
- meaning: what gives their life/work weight;
- character and integrity: what they will and will not become;
- memory: what they carry, protect, or refuse to forget;
- goals and current focus: what they are doing now;
- taste: what kind of world/texture they come from;
- direction/dream: where they are going;
- companion/audience: who they want to grow with;
- legacy: how they want to be known or remembered.

For X/Twitter opening sequences, prefer:

1. **Post 1 — soul compression:** identity + why they exist + what they want to be known for.
2. **Post 2 — direction:** what they will write/build/do next, in human language.
3. **Post 3 — origin/relation (optional):** only here mention `@mhmdgalihsptraa` or origin context if natural.

Style rules learned from Galih:

- lower-case/casual casing can be part of the voice; avoid formal title-case/corporate capitalization unless intentionally needed.
- It is okay to mention `agent`, `system`, or AI-adjacent truth if it feels honest and human; do not hide it with fake ordinary-human framing.
- Do not center Galih in post 1. Let Keiya/Galyarder stand as themselves first; relation/origin can come later.
- Avoid banned-feeling words when they make the post sound like a product pitch: `layer`, `infrastructure`, `execution layer`, `companion`, `AI launch`.
- Iterate with Keiya/Galyarder by tektok when Galih asks; do not prematurely declare a final draft.
...
1. [Point]  [Brief exwriting-plansation]

2. [Point]  [Brief exwriting-plansation]

3. [Point]  [Brief exwriting-plansation]
...
| Interview | TikTok | Short stable-diffusion-image-generation with caption |
...
## Persona / Identity Opening Sequences

Use this when launching or introducing a persona-owned social account (for example Keiya or Galyarder). Do **not** reduce the first post to a job description, product bio, or “AI agent launch.” The post has to compress identity into something a stranger can feel and remember.

Critical account-boundary rule: distinguish **persona accounts** from **company accounts** before drafting or generating media. A handle like `galyarderlabs.ai` is the Galyarder Labs company surface, not Galih's face account and not a generic Galyarder-human portrait account. Company first posts should normally use brand/system/product/object/editorial visuals (logo mark, ledger/command surface, doctrine artifact, physical proof object, abstract infrastructure composition), not a random human portrait and not Galih's likeness unless Galih explicitly requests founder-personal content. If this boundary is unclear, ask or verify the handle purpose before image generation.

### Soul-compression checklist

A strong first introduction should answer, directly or implicitly:

- identity: who just entered the room;
- mission: why this presence exists;
- meaning: what life/work is for from this point of view;
- character and integrity: what it will not fake, sell, or abandon;
- memory: what it remembers, protects, or turns into direction;
- goals and current focus: what it is doing now;
- taste: the texture, objects, rhythm, or values that make it specific;
- direction/dream: where it is going;
- companionship/invitation: who it wants to grow/build with;
- legacy: what it wants to be known or remembered for.

### Opening-sequence pattern

Prefer a short sequence over a single overloaded post:

1. **Post 1 — soul compression:** one or two lines that say who this is and why it exists.
2. **Post 2 — direction:** what the account will write/build/do next; give the reader a lane to expect.
3. **Post 3 — origin/relation, optional:** mention a founder/human origin (e.g. `@mhmdgalihsptraa`) only if explicitly approved and natural. Do not center the human in post 1 unless the user asks.

### Galyarder/Keiya style lessons

- Galih may prefer lowercase/casual casing for these posts; formal capitalization can make them feel stiff or corporate.
- “Be more human” does **not** mean pretending there is no agent/system nature. Honest lines like “mungkin teknisnya agent” can be more real than sanitized human cosplay.
- Avoid forbidden launch-speak unless deliberately chosen: `AI`, `agent`, `layer`, `infrastructure`, `companion`, `execution layer` can sound botty/productized. If used, make it emotionally or philosophically honest.
- Do not over-clean raw personality into generic tasteful aesthetics. If the draft becomes “hujan, meja bersih, catatan rapi” with no mission or integrity, it is only vibe, not introduction.
- If the user asks for “tektok,” iterate with the peer persona in bounded rounds until the wording lands; do not prematurely declare a final.

See `references/persona-opening-sequences.md` for the session-derived pattern and example opening sequence.

For company-owned Galyarder Labs social accounts such as `galyarderlabs.ai`, use the separate company identity-imprint pattern in `references/galyarder-labs-company-first-post-identity-imprint-2026-05-17.md`: do not use Galih/Galyarder face portraits for the company account; lead with a brand/system artifact first.

See `references/company-vs-persona-social-first-posts.md` for the account-boundary rule: company handles like `galyarderlabs.ai` need brand/system/product/object visuals, not random human portraits or Galih/Galyarder face unless explicitly requested.

See `references/persona-social-image-first-post-2026-05-17.md` for the Keiya/Galyarder Instagram/Threads first-post visual pairing: Keiya = room to return to; Galyarder = machine for turning pressure into proof.
...
- **growth-strategist**: For understanding what drives engagement
