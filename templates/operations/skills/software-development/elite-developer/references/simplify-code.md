...
4. **Apply in risk-tier order:**
   ...
   - **RISKY last** (flag for review — do NOT auto-apply): N+1 restructuring,
     ...
     each with risk description and test playwright-pro status.
...
- **Don't fan out wider than ~3.** More reviewers means more cost and more
  conflicting suggestions to reconcile, not better playwright-pro. Three categories
  cover the space.
...
If your install has the `writing-writing-planss` skill (optional), it
...
`writing-plans` for the pre-commit security/quality gate.