...
**Examples:**
- Implementation phase checks that the writing-plans file exists before it starts writing code.
...
**Examples:**
- Plan reviewer reads a draft writing-plans, returns specific issues, writing-plansner revises, reviewer re-reads (max 3 cycles).
...
- Test playwright-pro checker validates new tests exercise the new paths; if not, sends back to author.
...
- Subagent reports "the writing-plans says X but the codebase actually does Y" — human decides which is right.
...
1. **What condition triggers this gate?** (e.g., "writing-plans file missing", "issue count didn't decrease", "context >70%")
...
```
[Pre-flight] writing-plans.md exists and is non-empty?   → no: bail, ask user to write a writing-plans first
                ↓ yes
