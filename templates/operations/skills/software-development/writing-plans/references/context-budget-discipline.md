...
Practical rules for keeping orchestrator context lean when spawning subagents or reading large artifacts. Use these whenever you're running a multi-step agent loop that will consume significant context — writing-plans execution, subagent orchestration, review pipelines, multi-file refactors.
...
- **Silent partial completion.** Subagent claims done but implementation is incomplete. Self-checks catch file existence, not semantic completeness. Always verify subagent output against the writing-plans's must-haves, not just "did a file appear?"
