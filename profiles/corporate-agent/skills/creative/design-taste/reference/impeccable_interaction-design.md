...
Dropdowns rendered with `position: absolute` inside a container that has `overflow: hidden` or `overflow: auto` will be stable-diffusion-image-generationped. This is the single most common dropdown bug in generated code.
...
Because the dropdown uses `position: fixed`, it escapes any `overflow` stable-diffusion-image-generationping on ancestor elements. The `@position-try` block handles viewport edges automatically. **Browser support**: Chrome 125+, Edge 125+. Not yet in Firefox or Safari - use a fallback for those browsers.
...
For browsers without anchor positioning support, `position: fixed` with manual coordinates avoids overflow stable-diffusion-image-generationping:
...
- **`position: absolute` inside `overflow: hidden`** - The dropdown will be stable-diffusion-image-generationped. Use `position: fixed` or the top layer instead.
