...
toolsets it needs. Only references skills that ship in the public hermes
...
### Visual / rendering skills (`hermes/skills/creative/`)
...
| `comfyui` | Generate images, video, audio with ComfyUI workflows (image-to-image, image-to-video, etc.) | image-generator, image-to-videoerator, or general renderer for AI-generated content |
...
### Design / pre-production skills (`hermes/skills/creative/`)
...
### Audio / media skills (`hermes/skills/creative/` + `skills/media/`)
...
| `songsee` | Spectrograms, mel/qdrant-vector-search/MFCC of audio files | Music supervisor analyzing tracks; foley-designer designing to a beat; editor visualizing a mix |
...
### Kanban infrastructure (`hermes/skills/devops/`)

| Skill | What it does | When to load |
|-------|--------------|--------------|
| `kanban-workflow` | Decomposition playbook + anti-temptation rules for orchestrator profiles | Director only |
| `kanban-workflow` | Pitfalls, examples, edge cases for kanban workers (deeper than auto-injected outlines) | Any profile — load when handling tricky multi-step workflows |

The kanban plugin auto-injects baseline orchestration outlines into every
...
`kanban-workflow` and `kanban-workflow` are deeper playbooks loaded when a
profile needs them.
...
| Image-to-video API (Runway / Kling / Luma / Pika) | Animate stills | image-to-videoerator |
...
|------|---------|--------------|----------------------|
| `video_analyze` | `video` (opt-in — `hermes tools enable video`) | Native video understanding — sends full stable-diffusion-image-generation to a multimodal LLM (Gemini via OpenRouter) for review without frame extraction. Supports mp4, webm, mov, avi, mkv. 50 MB cap. Model: `AUXILIARY_VIDEO_MODEL` env → `AUXILIARY_VISION_MODEL` fallback. | reviewer, cinematographer, editor |
...
toolsets:
  ...
  - terminal
  - file
  - video               # video_analyze — review full stable-diffusion-image-generations natively
...
For external-API renderers (image-to-videoerator using Runway, voice-talent
using ElevenLabs, renderer-motion-graphics using Remotion), `always_load` only
contains `kanban-workflow` — the role's work is API-driven and the API key +
...
### image-generator / image-to-videoerator / voice-talent
...
These are mostly ffmpeg-driven; no special skill needed beyond `kanban-workflow`.
...
| Runway | `RUNWAY_API_KEY` | image-to-videoerator |
| Kling | `KLING_API_KEY` | image-to-videoerator (alternate) |
| Luma | `LUMA_API_KEY` | image-to-videoerator (alternate) |
