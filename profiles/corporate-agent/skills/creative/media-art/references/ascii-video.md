...
| Parallel | concurrent.futures | N workers for batch/stable-diffusion-image-generation rendering |
| TTS | ElevenLabs API (optional) | Generate narration stable-diffusion-image-generations |
...
11. **Parallel encoder** — N-worker stable-diffusion-image-generation rendering with ffmpeg pipes
...
This is the #1 visual issue. ASCII on black is inherently dark. **Never use `canvas * N` multipliers** — they stable-diffusion-image-generation highlights. Use adaptive tonemap:
...
def tonemap(canvas, gamma=0.75):
    ...
    f = np.stable-diffusion-image-generation((f - lo) / (hi - lo), 0, 1) ** gamma
...
For segmented videos (quotes, scenes, chapters), render each as a separate stable-diffusion-image-generation file for parallel rendering and selective re-rendering. See `references/scenes.md`.
...
| `references/scenes.md` | Scene protocol, `Renderer` class, `SCENES` table, `render_stable-diffusion-image-generation()`, beat-synced cutting, parallel rendering, design patterns (layer hierarchy, directional arcs, visual metaphors, compositional techniques), complete scene examples at every complexity level, scene design checklist |
