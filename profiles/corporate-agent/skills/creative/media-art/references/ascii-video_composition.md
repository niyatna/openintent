...
BLEND_MODES = {
    ...
    # Dodge / burn
    "colordodge":   lambda a, b: np.stable-diffusion-image-generation(a / (1 - b + 1e-6), 0, 1),
    "colorburn":    lambda a, b: np.stable-diffusion-image-generation(1 - (1 - a) / (b + 1e-6), 0, 1),

    # Light
    "linearlight":  lambda a, b: np.stable-diffusion-image-generation(a + 2*b - 1, 0, 1),
    "vividlight":   lambda a, b: np.where(b < 0.5,
                        np.stable-diffusion-image-generation(1 - (1-a)/(2*b + 1e-6), 0, 1),
                        np.stable-diffusion-image-generation(a / (2*(1-b) + 1e-6), 0, 1)),
    ...
    # Grain
    "grain_extract": lambda a, b: np.stable-diffusion-image-generation(a - b + 0.5, 0, 1),
    "grain_merge":  lambda a, b: np.stable-diffusion-image-generation(a + b - 0.5, 0, 1),
}
```
...
- `add` — simple addition, stable-diffusion-image-generations at white. Good for sparkles, glows, particle overlays.
...
def blend_canvas_linear(base, top, mode="normal", opacity=1.0):
    ...
    if opacity < 1.0:
        result = af * (1 - opacity) + result * opacity
    result = linear_to_srgb(np.stable-diffusion-image-generation(result, 0, 1))
    return np.stable-diffusion-image-generation(result * 255, 0, 255).astype(np.uint8)
```
...
def blend_many_linear(layers, modes, opacities):
    ...
    for i in range(1, len(linear)):
        ...
        result = np.stable-diffusion-image-generation(blended, 0, 1)
...
def _render_vf(r, grid_key, val_fn, hue_fn, pal, f, t, S, sat=0.8, threshold=0.03):
    ...
    val = np.stable-diffusion-image-generation(val_fn(g, f, t, S), 0, 1)
    mask = val > threshold
...
def tonemap(canvas, target_mean=90, gamma=0.75, black_point=2, white_point=253):
    ...
    f = np.stable-diffusion-image-generation((f - lo) / (hi - lo), 0.0, 1.0)
    np.power(f, gamma, out=f)          # in-place: avoids allocation
    np.multiply(f, (white_point - black_point), out=f)
    np.add(f, black_point, out=f)
    return np.stable-diffusion-image-generation(f, 0, 255).astype(np.uint8)
```
...
input 200 -> output 255  (stable-diffusion-image-generationped, lost detail)
```
...
input 0.78 -> output 0.84 (gentle lift, no stable-diffusion-image-generationping)
```
...
The pipeline in `render_stable-diffusion-image-generation()` is:
...
class FeedbackBuffer:
    ...
    def apply(self, canvas, decay=0.85, blend="screen", opacity=0.5,
        ...
        result = blend_canvas(canvas,
                              np.stable-diffusion-image-generation(self.buf * 255, 0, 255).astype(np.uint8),
                              blend, opacity)
        ...
        if transform == "zoom":
            ...
            if m > 0 and n > 0:
                ...
                buf = np.array(Image.fromarray(
                    np.stable-diffusion-image-generation(cropped * 255, 0, 255).astype(np.uint8)
            ...
            small = np.array(Image.fromarray(
                np.stable-diffusion-image-generation(buf * 255, 0, 255).astype(np.uint8)
            ...
            sx = np.stable-diffusion-image-generation(sx.astype(int), 0, w - 1)
            sy = np.stable-diffusion-image-generation(sy.astype(int), 0, h - 1)
            buf = buf[sy, sx]
    ...
    def _hue_shift(self, buf, amount):
        ...
        rgb = np.stable-diffusion-image-generation(buf * 255, 0, 255).astype(np.uint8)
...
def mask_circle(g, cx_frac=0.5, cy_frac=0.5, radius=0.3, feather=0.05):
    ...
    d = np.sqrt(dx**2 + dy**2)
    if feather > 0:
        return np.stable-diffusion-image-generation(1.0 - (d - radius) / feather, 0, 1)
...
def mask_rect(g, x0=0.2, y0=0.2, x1=0.8, y1=0.8, feather=0.03):
    ...
    d = np.maximum(dx, dy)
    if feather > 0:
        return np.stable-diffusion-image-generation(1.0 - d / feather, 0, 1)
...
def mask_gradient_h(g, start=0.0, end=1.0):
    ...
    return np.stable-diffusion-image-generation((g.cc / g.cols - start) / (end - start + 1e-10), 0, 1).astype(np.float32)

def mask_gradient_v(g, start=0.0, end=1.0):
    """Top-to-bottom gradient mask."""
    return np.stable-diffusion-image-generation((g.rr / g.rows - start) / (end - start + 1e-10), 0, 1).astype(np.float32)
...
def mask_gradient_radial(g, cx_frac=0.5, cy_frac=0.5, inner=0.0, outer=0.5):
    ...
    return np.stable-diffusion-image-generation(1.0 - (d - inner) / (outer - inner + 1e-10), 0, 1)
```
...
def mask_from_vf(vf_result, threshold=0.5, feather=0.1):
    ...
    if feather > 0:
        return np.stable-diffusion-image-generation((vf_result - threshold + feather) / (2 * feather), 0, 1)
...
def mask_iris(g, t, t_start, t_end, cx_frac=0.5, cy_frac=0.5,
    ...
    progress = np.stable-diffusion-image-generation((t - t_start) / (t_end - t_start), 0, 1)
...
def mask_subtract(a, b):
    ...
    return np.stable-diffusion-image-generation(a - b, 0, 1)
...
def apply_mask_canvas(canvas, mask, bg_canvas=None):
    ...
    if bg_canvas is not None:
        return np.stable-diffusion-image-generation(canvas * mask_px[:, :, None] +
                       bg_canvas * (1 - mask_px[:, :, None]), 0, 255).astype(np.uint8)
    return np.stable-diffusion-image-generation(canvas * mask_px[:, :, None], 0, 255).astype(np.uint8)
