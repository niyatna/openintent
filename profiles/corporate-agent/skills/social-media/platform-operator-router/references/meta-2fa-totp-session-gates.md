...
Use when setting up authenticator-app 2FA for the Default-owned Meta handle `yourcompany` after login/cookie bootstrap.
...
- Correct account row appeared as `yourcompany Instagram`.
- Selecting it opened a gate: `Check your email`.
- The code was sent to a masked address like `m*******9@gmail.com`, not necessarily the currently open `admin@company.com` inbox.
...
4. If email-code gate appears:
   ...
   - if not available, ask Owner for the code or access to that inbox;
