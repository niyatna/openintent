...
Linear is configured as a remote OAuth MCP server in both Co-Founder/default and Default profiles:
...
**Verified live in Default session 2026-05-20:** `mcp_linear_get_user({query: "me"})` returned the Linear user `developer@company.com`, admin, active, team `Default` key `GAL`.
...
- create/update private/internal issues when Owner explicitly asks;
- comment with a status update when Owner explicitly asks.
...
- posting as Owner externally if content affects public/customer/investor communication.
