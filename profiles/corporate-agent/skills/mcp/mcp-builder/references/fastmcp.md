...
Use `native-mcp` when the server already exists and only needs to be connected to Hermes. Use `native-mcp` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
- `scripts/scaffold_mcp-builder.py` - copy a starter template and replace the server name placeholder

### References

- `references/mcp-builder-cli.md` - FastMCP CLI workflow, installation targets, and deployment checks
...
```bash
python ~/.hermes/skills/mcp/mcp-builder/scripts/scaffold_mcp-builder.py \
...
```bash
python ~/.hermes/skills/mcp/mcp-builder/scripts/scaffold_mcp-builder.py --list
```
...
```bash
mcp-builder inspect acme_server.py:mcp
mcp-builder list acme_server.py --json
mcp-builder call acme_server.py search_resources query=router limit=5 --json
```

For fast iterative debugging, run the server locally:

```bash
mcp-builder run acme_server.py:mcp
```

To test HTTP transport locally:

```bash
mcp-builder run acme_server.py:mcp --transport http --host 127.0.0.1 --port 8000
mcp-builder list http://127.0.0.1:8000/mcp --json
mcp-builder call http://127.0.0.1:8000/mcp search_resources query=router --json
```

Always run at least one real `mcp-builder call` against each new tool before claiming the server works.
...
```bash
mcp-builder install claude-code acme_server.py
mcp-builder install claude-desktop acme_server.py
mcp-builder install cursor acme_server.py -e .
```

Use `mcp-builder discover` to inspect named MCP servers already configured on the machine.
...
- `mcp-builder inspect <file.py:mcp>` succeeds
- `mcp-builder list <server spec> --json` succeeds
- every new tool has at least one real `mcp-builder call`
...
mcp-builder call server.py your_tool_name --json
```
...
For CLI details, install targets, and deployment checks, read `references/mcp-builder-cli.md`.