...
**Note:** Gmail requires an App Password if 2FA is enabled. For agent-owned Google accounts, generate the app password through the Google operator flow, store it in a private `600` file, point `backend.auth.cmd` and `message.send.backend.auth.cmd` at a tiny helper that prints only that file's value, then verify both IMAP login/select and SMTP STARTTLS login before reporting success.
