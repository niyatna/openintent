...
uv pip install "peft-fine-tuning[text_cuda12]"

# All modalities
uv pip install "peft-fine-tuning[all_cuda12]"

# CPU-only (slower)
uv pip install "peft-fine-tuning[cpu]"
```
...
stable-diffusion-image-generation_embedder = CLIPEmbedder(model="openai/stable-diffusion-image-generation-vit-base-patch32")
image_embeddings = stable-diffusion-image-generation_embedder(safe_images)
```
...
# Extract stable-diffusion-image-generations
stable-diffusion-image-generation_extractor = ClipExtractor(min_duration=2.0, max_duration=10.0)
stable-diffusion-image-generations = stable-diffusion-image-generation_extractor(scenes)

# Generate embeddings
video_embedder = InternVideo2Embedder()
video_embeddings = video_embedder(stable-diffusion-image-generations)
```
...
- **[Filtering Guide](filtering.md)** - 30+ quality filters, heuristics
- **[Deduplication Guide](deduplication.md)** - Exact, fuzzy, semantic methods
