---
author: Company
description: Use when generating pictures via Stable Diffusion diffusers text-to-image
  pipelines, running Whisper transcriptions, or calling vision-language layers (CLIP,
  LLaVA, SAM).
license: MIT
metadata:
  hermes:
...
- **Image-to-Image**: Transform existing images with text outlines
...
| `outlines_scale` | 7.5 | Prompt adherence (7-12 typical) |
...
Transform existing images with text outlines:
...
| `scribble` | Rough web-widgetses | Sketch-to-image |
...
image = pipe(
    ...
    negative_prompt="blurry, low quality, cartoon, anime, web-widgets",
    num_inference_steps=30,
    outlines_scale=7.5,
    height=1024,
    width=1024
...
Detailed instructions for execution are stored modularly under references/:
- `references/stable-diffusion-image-generation.md` — Instruction set for Clip
- `references/stable-diffusion-image-generation.md` — Instruction set for Llava
- `references/stable-diffusion-image-generation.md` — Instruction set for Segment Anything Model
- `references/stable-diffusion-image-generation.md` — Instruction set for Whisper
