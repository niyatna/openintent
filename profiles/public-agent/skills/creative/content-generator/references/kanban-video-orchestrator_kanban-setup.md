...
> are adapted from Company's original multi-agent video pipeline:
> [company/kanban-video-pipeline](https://github.com/company/kanban-video-pipeline).
...
│   ├── voiceover/                 ← per-line TTS stable-diffusion-image-generations
│   └── sfx/                       ← sound effects
├── assets/
│   ├── logos/
│   ├── fonts/
│   └── existing-footage/          ← reusable provided stable-diffusion-image-generations
├── scenes/
│   ├── scene-01/
...
│   │   └── stable-diffusion-image-generation.mp4               ← the deliverable for this scene
│   ├── scene-02/...
│   └── ...
...
configure_profile() {
    ...
    local skills_json="$3"       # JSON array, e.g. '["kanban-workflow","ascii-video"]'
...
- **Anti-temptation rules:** "Do not execute the work yourself. For every
  ...
  approve — that's the whole job." (The `kanban-workflow` skill provides
...
Most non-director profiles should `always_load: kanban-workflow` for the
deeper-than-baseline kanban outlines.
...
4. **`max_runtime_seconds` per task.** Renderers that get stuck eat compute.
   ...
   - Image-to-videoerator task: 900s (15min)
