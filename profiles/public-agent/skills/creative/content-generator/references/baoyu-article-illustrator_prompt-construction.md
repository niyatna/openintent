...
Colors in prompts use hex codes for **rendering outlines only** — they tell the model which colors to use, NOT what text to display.
...
> Color values (#hex) and color names are rendering outlines only — do NOT display color names, hex codes, or palette labels as visible text in the image.
