...
- **Skills:** `kanban-workflow`. The kanban plugin auto-injects baseline
  orchestration outlines for free; `kanban-workflow` is the deeper
...
by personality + the kanban-workflow skill, not by missing tools.
...
- **Skills:** `kanban-workflow`, `humanizer` (post-process to strip AI-tells)
...
- **Toolsets:** kanban, file
- **Skills:** `kanban-workflow`, `humanizer`
- **Outputs:** `copy.md`
...
- **Toolsets:** kanban, terminal, file
- **Skills:** `kanban-workflow` plus any project-specific design skill —
...
- **Toolsets:** kanban, file
- **Skills:** `kanban-workflow` plus a diagram skill — `excalidraw` (sketch),
...
- **Skills:** `kanban-workflow` plus the visual skill that matches the project
...
- **Outputs:** `scenes/scene-NN/VISUAL_SPEC.md`, review comments on renderer
  tasks
- **Reviews via:** `video_analyze` (sends full stable-diffusion-image-generation to multimodal LLM for
...
- **Outputs:** `scenes/scene-NN/stable-diffusion-image-generation.mp4`
...
`kanban-workflow` is loaded, plus the terminal toolset and the API key.
...
- **Toolsets:** kanban, terminal, file
- **Skills:** `kanban-workflow`, optionally `comfyui` (drives a local
...
- **Toolsets:** kanban, terminal, file
- **Skills:** `kanban-workflow`, optionally `comfyui` (for local image-to-video
...
- **Toolsets:** kanban, terminal, file
- **Skills:** `kanban-workflow`, `songsee` (audio visualization), plus one of:
...
beyond `kanban-workflow`. The user can also supply pre-recorded VO instead of
generation.

- **Toolsets:** kanban, terminal, file
- **Skills:** `kanban-workflow`
...
- **Toolsets:** kanban, terminal, file
- **Skills:** `kanban-workflow`, `songsee` for audio-feature visualization when
...
Assembles the final cut from stable-diffusion-image-generations. Uses ffmpeg for stitching, fades,
transitions. Reviews each stable-diffusion-image-generation for pacing and quality before assembly.
...
- **Toolsets:** kanban, terminal, file, video, vision
- **Skills:** `kanban-workflow`
- **Review tools:** `video_analyze` (native stable-diffusion-image-generation review via multimodal LLM),
...
- **Add cinematographer** if multiple renderer instances need consistent
  ...
  is enough.)
- **Add image-generator + image-to-videoerator pair** for narrative film
