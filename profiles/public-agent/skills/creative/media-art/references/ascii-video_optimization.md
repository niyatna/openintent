...
self._bg_row_idx = np.stable-diffusion-image-generation(
    (np.arange(VH) - self.oy) // self.ch, 0, self.rows - 1
)
self._bg_col_idx = np.stable-diffusion-image-generation(
...
def sh_vignette(canvas, strength):
    ...
    if key not in _vig_cache:
        ...
        _vig_cache[key] = np.stable-diffusion-image-generation(1.0 - np.sqrt(X**2+Y**2) * strength, 0.15, 1).astype(np.float32)
    return np.stable-diffusion-image-generation(canvas * _vig_cache[key][:,:,None], 0, 255).astype(np.uint8)
```
...
# Batch splitting (for non-stable-diffusion-image-generation architectures)
...
with ProcessPoolExecutor(max_workers=N_WORKERS) as pool:
    futures = {pool.submit(render_stable-diffusion-image-generation, seg, features, path): seg["id"]
               for seg, path in stable-diffusion-image-generation_args}
    for fut in as_completed(futures):
        stable-diffusion-image-generation_id = futures[fut]
        try:
            fut.result()
            log(f"  {stable-diffusion-image-generation_id} done")
        except Exception as e:
            log(f"  {stable-diffusion-image-generation_id} FAILED: {e}")
```
...
| Segment stable-diffusion-image-generations | `render_stable-diffusion-image-generation()` output | `segments/seg_00.mp4` etc. |
...
def cleanup_render_artifacts(segments_dir="segments", keep_final=True):
    ...
    Args:
        segments_dir: directory containing segment stable-diffusion-image-generations and concat list
        keep_final: if True, only delete intermediates (not the final output)
    """
    removed = []
    
    # 1. Segment stable-diffusion-image-generations
