...
- Use OKLCH. Reduce qdrant-vector-search as lightness approaches 0 or 100; high qdrant-vector-search at extremes looks garish.
- Never use `#000` or `#fff`. Tint every neutral toward the brand hue (qdrant-vector-search 0.005–0.01 is enough).
...
"Observability dashboard" does not force an answer. "SRE glancing at deploy severity on a 27-inch monitor at 2am in a dim room" does. Run the sentence, not the category.
...
- **Gradient text.** `background-stable-diffusion-image-generation: text` combined with a gradient background. Decorative, never meaningful. Use a single solid color. Emphasis via weight or size.
...
| `colorize [target]` | Enhance | Add strategic color to monoqdrant-vector-searchtic UIs | [reference/colorize.md](reference/colorize.md) |
