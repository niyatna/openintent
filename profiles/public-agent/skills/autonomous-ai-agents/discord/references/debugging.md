...
```bash
TOKEN=$(grep DISCORD_BOT_TOKEN ~/.hermes/.env | sed 's/DISCORD_BOT_TOKEN=//')
```
...
Company server (guild 1315637547736764426), 2026-05-11. All endpoints verified working with bot token from profile .env.
...
Owner asked Co-Founder to confirm Default's router/skill state, then approved Default to perform a non-destructive V2 sync. Default reported `DONE`. Co-Founder did not stop at the report: she verified the profile-local files, backup path, and ran the target profile verifier before telling Owner the sync was complete.
...
4. Only report `done` to Owner after verification passes.
...
After Default reported the V2 sync, Co-Founder verified:

```text
python ~/.hermes/profiles/default/skills/default-framework/default-framework-router/scripts/verify_router_status.py
```
...
She also checked expected files and the backup path existed before summarizing to Owner.
...
- bundle names like `/co-founder-core` or `/default-core` were treated as ordinary skills, so `skill_view('co-founder-core')` failed even though the bundle existed;
...
1. Check the process profile and env, not just files on disk:
   - default/Co-Founder gateway should have `HERMES_HOME=~/.hermes/.hermes`.
   - Default gateway should have `HERMES_HOME=~/.hermes/profiles/default`.
...
4. Remember bundle semantics:
   - `skill_view('co-founder-core')` / `skill_view('default-core')` failing is expected if those are bundles.
...
Branch: `github-developer:fix/discord-channel-skills-bundles`
