...
This Default profile has the package installed and the MCP server configured.
...
```bash
repo=~/.hermes/notebooklm-mcp-cli
nlm=~/.hermes/.local/bin/nlm
notebooklm_mcp=~/.hermes/.local/bin/notebooklm-mcp
package=notebooklm-mcp-cli==0.6.13
credentials=~/.hermes/.notebooklm-mcp-cli/profiles/default
...
mcp_servers:
  notebooklm:
    command: ~/.hermes/.local/bin/notebooklm-mcp
    enabled: true
```

Important: this workstation uses the single-home rule. Do **not** recreate `~/.hermes/profiles/default/home`. Prefer OS-home absolute paths and set `HOME=~/.hermes` in shell checks:
...
For this Default profile, use raw OS-home `nlm` for auth and troubleshooting:

```bash
~/.hermes/.local/bin/nlm <command>
```

Older references to profile-local `nlm-default` or `~/.hermes/profiles/default/home/...` are historical only. Under the current single-home setup, account-sensitive checks should use:

```bash
HOME=~/.hermes ~/.hermes/.local/bin/nlm login --check
HOME=~/.hermes ~/.hermes/.local/bin/nlm notebook list --json
```
...
Current canonical NotebookLM state should be treated as live-verification-dependent, not memory-only. A concise source map, when maintained, lives in Obsidian at `~/.hermes/Obsidian/company-labs/notebooklm-source-map.md`.
...
Validated smoke-test and use-case recipes live in `references/default-smoke-tests-and-use-cases.md`.
...
Default profile auth from arbitrary shells:
...
Use OS-home `HOME=~/.hermes` explicitly. The old profile-local `nlm-default` wrapper belonged to the deleted profile-home setup and must not be recreated.
...
- Account identity matters and MCP results conflict with what Owner expects.

Default profile correction: if MCP NotebookLM tools show a stale/different notebook list, verify with the OS-home CLI before answering:
...
If `nlm login` cannot find a browser but Brave/Camofox exists, keep `HOME=~/.hermes`, inspect browser availability, and use manual cookie/browser fallback without creating `~/.hermes/profiles/default/home`. The old `nlm-default` wrapper path is historical only.
