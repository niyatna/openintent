...
Use this when promoting a dedicated agent-owned GitHub browser account into CLI/API access for Hermes profiles such as Co-Founder or Default.
...
- Keep Owner's main `gh` auth separate from agent-owned GitHub identities.
- Use one fine-grained PAT per dedicated agent account/profile.
- Store the PAT only in a local private token file such as:
  - `~/.hermes/private/credentials/agents/<owner>/github/token.env`
...
Observed during dedicated Co-Founder/Default token setup:

- Opening the fine-grained PAT creation page from an active browser session can still redirect to **Confirm access** / sudo mode.
- Co-Founder reached sudo email verification as the dedicated account; automated Gmail code retrieval submitted a stale or wrong code once and GitHub returned `Sudo authentication failed`.
- Default reached the fine-grained PAT form as the dedicated account; URL query parameters prefilled the intended permissions, but form submission did not return a token in the headless automation attempt.
- Do **not** try to get a dedicated-account token via `gh auth login --web` when the OS/user browser is logged into Owner's human GitHub account; GitHub CLI device/web OAuth will authorize the currently active browser account and can silently produce a token for the wrong login. If a wrong-account token is captured, delete it immediately and verify no token remains.
- If fine-grained PAT automation fails, use the dedicated account's GitHub web settings page for a classic PAT (`https://github.com/settings/tokens/new`) as the direct fallback, scoped minimally (e.g. `repo`, `workflow`, `read:user`) and then verify with the per-agent wrapper. Classic PAT worked for Co-Founder/Default when fine-grained PAT form automation redirected to `/repos` without exposing a token.
