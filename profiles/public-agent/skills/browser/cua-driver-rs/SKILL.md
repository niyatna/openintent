...
- `references/linux-multi-monitor-positioning.md` — guidelines for diagnosing and recovering Xwayland windows stuck on virtual workspaces or off-screen displays (workspace index -1 issue).
...
### Linux (GNOME/Wayland) panel & workspace freezes during computer-use
A common failure mode on GNOME Wayland desktop environment during automated interaction is the shell panel, top bar, or workspace overlay freezing/locking up due to window overrides and graphics pipeline locking. 

If after `computer-use` execution, the desktop state is unresponsive, or the top bar/dock becomes hidden or non-functional:
1. **Force kill** any foreground automation windows that may be holding absolute redirect locks (e.g., interactive browser profiles).
2. **Recover GNOME Shell**: Since GNOME Shell cannot be restarted inside a native Wayland session via `gnome-shell --replace` directly (since Wayland treats the compositor as the display server itself), use:
   ```bash
   killall -HUP gnome-shell
   ```
   Or if that fails to reload the panel:
   ```bash
   export DISPLAY=:0 && gnome-shell --wayland
   ```
   (run as a background process session).
3. **Verify/Restore interface extensions settings**:
   Often, tweak extensions (like `just-perfection-desktop`) can have their state set to hidden or locked during crashes. Query and restore key parameters using `gsettings` with explicit local schema paths if globally unregistered:
   ```bash
   ...
   gsettings --schemadir ~/.local/share/gnome-shell/extensions/just-perfection-desktop@just-perfection/schemas/ set org.gnome.shell.extensions.just-perfection activities-button true
   gsettings --schemadir ~/.local/share/gnome-shell/extensions/just-perfection-desktop@just-perfection/schemas/ set org.gnome.shell.extensions.just-perfection overlay-key true
   ```
4. **Restore default input settings**: Enable system hot-corners and interface controls:
   ```bash
...
- **Never bypass native tools for shell hacks**: Do not run experimental nested window managers (`gnome-shell --wayland --nested`) or dirty DBus evals to recover or bypass shell freezes.
- **Prefer direct CLI processes over visual GUI clicks for app/media lifecycle**: Controlling application state (opening/closing tabs/windows, setting system volume, playing media) must always use deterministic shell commands (like `kill <pid>`, launching Brave targeting the target URL directly, `wpctl`, `playerctl`) rather than visual cursor steering or keystroke emulation loops. Keep `computer_use` reserved for tasks without CLI equivalents.
- **Chromium/Brave PWA tab activation workaround**: Launching Brave PWA using the `--app-id` flag when the browser is already running can result in loading the default start URL (home screen) and completely ignoring the trailing link parameters. To open or play a specific media/link directly, launch Brave using standard window commands with the profile directory flag (`--profile-directory="Profile 3" "https://..."`) in a tab, and then send a `click` command to the inactive/suspended tab's `element_index` (which may show a label containing "Memory usage - [N] MB") to bring it into focus and enable audio/video playback.
