...
Use Python/openpyxl. Write formula strings (`ws["D20"] = "=B5*B6"`), then run the `xlsx` skill's `recalc.py` helper before delivery.
...
```bash
python /path/to/xlsx/scripts/recalc.py model.xlsx
```
