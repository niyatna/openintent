...
Session learning from normalizing all `default-framework` skills into Hermes Agent style.
...
1. Resolve both roots explicitly, for example `~/.hermes/skills` and `~/.hermes/profiles/default/skills`.
...
5. For persona/media skills, verify profile-local asset paths. In the Default profile, image references live under `~/.hermes/profiles/default/image_cache/`, with separate subdirs for `default/` and `co-founder-co-founder/`.
