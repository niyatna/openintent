...
Use this skill when Owner asks any of these:
...
Robust resolver for Owner's environment:

```bash
CG="${CODEGRAPH_BIN:-$(command -v codegraph || true)}"
if [ -z "$CG" ] && [ -x ~/.hermes/.nvm/versions/node/v24.15.0/bin/codegraph ]; then
  CG=~/.hermes/.nvm/versions/node/v24.15.0/bin/codegraph
fi
