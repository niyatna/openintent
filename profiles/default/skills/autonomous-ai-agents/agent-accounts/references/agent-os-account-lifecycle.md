...
Use this as the umbrella playbook for promoting Co-Founder/Default account skeletons into operational dedicated agent-owned identities.
...
```bash
HERMES_HOME=~/.hermes/.hermes ~/.hermes/scripts/agent-os-quick access-hardening
```

Expected: `status=pass`, no errors, no warnings. If X has an approved populated password but status starts with `x-login-blocked-`, the verifier must classify it as operational credential state, not a planned skeleton violation.
...
If `actual_login` is `developer-account`, stop: a human-account token leaked into the dedicated flow. Remove it from the agent token file and redo via the dedicated account's GitHub web token page.
...
No wallet spend/signing until Owner defines:
...
Both Co-Founder and Default SOUL files should include the concepts below. Exact heading names are useful for verifiers, but do not confuse a missing heading with a missing concept after SOUL compression; check both text semantics and verifier invariants.
...
Do not blindly adopt `user account = agent account` as a global rule for Owner. Treat it as a design option for a dedicated single-user agent, not as default policy for Co-Founder/Default.

## Common pitfalls

1. Treating GitHub browser login as API access. It is not; wrapper check is required.
2. Using `gh auth login --web` for dedicated accounts while the real browser is logged into Owner's human account. This can mint a wrong-account token.
