...
- `references/agent-accounts-reference.md` — Consolidated guide for managing and verifying dedicated agent-owned credentials, GitHub PAT tokens, two-factor authentication, and Bitwarden.
- `references/agent-os-account-lifecycle.md` — Conceptual modeling checklist for active credentials promotion and operational lifecycles.
## Overview

This skill is for **dedicated agent-owned accounts**, not Owner's primary personal accounts.

Core rule: login automation is allowed only after the account has an access contract, safe credential storage, confirmation thresholds, and a read-only/safe-write test. CloakBrowser persistent profiles are the primary isolated auth/session route. Camofox/Camofox is legacy/fallback only when CloakBrowser is blocked or the platform skill still has a verified Camofox-only CRUD flow. Secrets stay in private credential files or an encrypted manager, never in chat, SOUL, memory, Obsidian, skills, profile repos, or logs.

## When to Use

Use this when Owner asks to:

- prepare Co-Founder/Default-owned Google, GitHub, X/Twitter, Notion/Linear OAuth, or similar accounts;
...
Do not use this for mass signup, CAPTCHA bypass, OTP/email farm automation, quota evasion, scraping behind access controls, or accounts Owner does not own/control.
...
```text
~/.hermes/private/credentials/agents/
  co-founder/
    google/account.txt
    github/account.txt
    x/account.txt
...
BACKUP_CODES_FILE=~/.hermes/private/credentials/agents/co-founder/google/backup-codes.txt
COOKIES_FILE=~/.hermes/private/credentials/agents/co-founder/google/cookies.json
RECOVERY_EMAIL=
PHONE_LAST4=
...
```bash
python ~/.hermes/profiles/default/skills/autonomous-ai-agents/agent-owned-account-login/scripts/account_totp.py \
  --account-file ~/.hermes/private/credentials/agents/co-founder/google/account.txt
```
...
```bash
python ~/.hermes/profiles/default/skills/autonomous-ai-agents/agent-owned-account-login/scripts/account_init.py \
  --owner co-founder \
  --service google \
  --email co-founder.example@gmail.com
```
...
Validator/audit scripts should check the fields the initializer actually emits: `ACCOUNT_ID=`, `OWNER=`, `SERVICE=`, empty secret placeholders, and `NOTES=dedicated agent-owned account`. Do not require a synthetic `STATUS=planned-placeholder` unless the initializer is changed to write it; that causes false-positive hardening failures.

Only fill real credentials locally after the account is approved and the access contract exists in `~/.hermes/private/credentials/access-registry.yaml`.
