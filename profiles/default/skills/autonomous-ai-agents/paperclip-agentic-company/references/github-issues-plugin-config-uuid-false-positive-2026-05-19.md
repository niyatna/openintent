...
Paperclip's config validation endpoint scans ALL config values for UUID patterns (36-char with dashes). When a binding includes `companyId` with a UUID value like `43bf734f-bcf3-498a-b86e-a9f9db1418fd`, the backend treats it as a secret reference and rejects the entire config save.
...
Worker file: `~/.hermes/.paperclip/plugins/node_modules/@wil0x91/paperclip-plugin-github-issues/dist/worker.js`
...
- `companyId` — Paperclip company ID (omit to avoid UUID false-positive)
- `projectId` — optional Paperclip project
- `labelPrefix` — optional label prefix for synced issues
- `defaultStatus` — initial Paperclip status for imported issues (default: `todo`)
