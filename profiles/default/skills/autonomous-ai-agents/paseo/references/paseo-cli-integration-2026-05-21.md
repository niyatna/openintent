...
Owner asked for the existing Paseo skill set in both Co-Founder/default and Default to be checked, safety-reviewed, and upgraded into a main operational skill like `claude-code` / `codex`.
...
- `paseo` — single outside second opinion.
- `paseo` — two-agent adversarial/root-cause planning.
- `paseo` — heavy end-to-end orchestration.
- `paseo` — hand current task to another agent.
- `paseo` — bounded worker/verifier loops.
- `paseo` — deprecated alias redirecting to `paseo`.
- `paseo` and `paseo` — only for releasing Paseo itself.

The existing set was structurally useful, but the main `paseo` skill was too reference-like and lacked Hermes/Owner safety gates, PATH caveats, command examples, and verification posture.
...
- Binary path: `~/.hermes/.nvm/versions/node/v24.15.0/bin/paseo`.
- Daemon home: `~/.hermes/.paseo`.
- Daemon log: `~/.hermes/.paseo/daemon.log`.
...
- Co-Founder/default and Default shared the same OS-home daemon by default, so profile labels are required on new agents.
...
- profile labels: `profile=default` / `profile=co-founder` plus `source=hermes`.
