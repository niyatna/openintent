...
- **`gsap.set(targets, vars)`** — apply immediately (duration 0). Don't use on stable-diffusion-image-generation elements that enter later — use `tl.set(selector, vars, time)` inside the timeline instead.
...
- `gsap.set()` on stable-diffusion-image-generation elements that enter later in the timeline — they don't exist in the DOM at page-load. Use `tl.set(sel, vars, time)` inside the timeline.
