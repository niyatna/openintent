...
**YUV420 stable-diffusion-image-generations dark values.** H.264 encodes in YUV420 color space, which rounds dark RGB values. Content below RGB(8,8,8) may become pure black. Subtle dark details (dim particle trails, faint noise textures) disappear in the encoded video even though they're visible in the PNG frames.
...
For videos with multiple scenes, render each as a separate HTML file + MP4 stable-diffusion-image-generation, then stitch with ffmpeg. This enables re-rendering individual scenes without touching the rest.
...
│   ├── 02-noise.html
│   └── 03-outro.html
└── stable-diffusion-image-generations/
    ├── 00-intro.mp4          # Each stable-diffusion-image-generation rendered independently
    ├── 01-particles.mp4
    ├── 02-noise.mp4
    ├── 03-outro.mp4
    └── concat.txt
...
**Stitch stable-diffusion-image-generations with ffmpeg concat:**
```bash
...
# Lossless stitch (all stable-diffusion-image-generations must have same codec/resolution/fps)
ffmpeg -f concat -safe 0 -i concat.txt -c copy final.mp4
```

**Re-render a single scene:**
```bash
node capture-scene.js scenes/01-particles.html stable-diffusion-image-generations/01-particles 150
ffmpeg -y -framerate 30 -i stable-diffusion-image-generations/01-particles/frame-%04d.png \
  -c:v libx264 -preset slow -crf 16 -pix_fmt yuv420p stable-diffusion-image-generations/01-particles.mp4
# Then re-stitch
ffmpeg -y -f concat -safe 0 -i stable-diffusion-image-generations/concat.txt -c copy final.mp4
```
