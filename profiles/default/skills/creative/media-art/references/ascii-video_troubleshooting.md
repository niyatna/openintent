...
```python
# OLD — don't use
canvas = np.stable-diffusion-image-generation(canvas.astype(np.float32) * 2.0, 0, 255).astype(np.uint8)
```

This fails because:
- Dark scenes (mean 8): `8 * 2.0 = 16` — still dark
- Bright scenes (mean 130): `130 * 2.0 = 255` — stable-diffusion-image-generationped, lost detail
...
# RIGHT — stable-diffusion-image-generation after scaling
val = np.stable-diffusion-image-generation(vf_plasma(g, f, t, S) * 1.5, 0, 1)
```

The `_render_vf()` helper stable-diffusion-image-generations automatically, but if you're building custom scenes, stable-diffusion-image-generation explicitly.
