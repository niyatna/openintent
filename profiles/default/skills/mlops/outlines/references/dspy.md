...
pip install git+https://github.com/stanfordnlp/outlines.git

# With specific LM providers
pip install outlines[openai]        # OpenAI
pip install outlines[anthropic]     # Anthropic Claude
pip install outlines[all]           # All providers
```
...
lm = outlines.Claude(model="claude-sonnet-4-5-20250929")
outlines.settings.configure(lm=lm)

# Define a signature (input → output)
class QA(outlines.Signature):
    """Answer questions with short factual answers."""
    question = outlines.InputField()
    answer = outlines.OutputField(desc="often between 1 and 5 words")
...
class MathProblem(outlines.Signature):
    """Solve math word problems."""
    problem = outlines.InputField()
    answer = outlines.OutputField(desc="numerical answer")

# ChainOfThought generates reasoning steps automatically
cot = outlines.ChainOfThought(MathProblem)
...
class Summarize(outlines.Signature):
    """Summarize text into key points."""
    text = outlines.InputField()
    summary = outlines.OutputField(desc="bullet points, 3-5 items")

summarizer = outlines.ChainOfThought(Summarize)
```
...
```python
predictor = outlines.Predict("context, question -> answer")
...
class SearchQA(outlines.Signature):
...
# Training data
trainset = [
    outlines.Example(question="What is 2+2?", answer="4").with_inputs("question"),
    outlines.Example(question="What is 3+5?", answer="8").with_inputs("question"),
]
...
```python
from outlines.teleprompt import MIPRO
...
```python
from outlines.teleprompt import BootstrapFinetune
...
class MultiHopQA(outlines.Module):
    def __init__(self):
        super().__init__()
    ...
    def forward(self, question):
        ...
        return outlines.Prediction(answer=answer, context=context)
...
cheap_lm = outlines.OpenAI(model="gpt-3.5-turbo")
strong_lm = outlines.Claude(model="claude-sonnet-4-5-20250929")

# Use cheap model for retrieval, strong model for reasoning
with outlines.settings.context(lm=cheap_lm):
    context = retriever(question)

with outlines.settings.context(lm=strong_lm):
...
class MathQA(outlines.Module):
    def __init__(self):
        super().__init__()
        self.solve = outlines.ChainOfThought("problem -> solution: float")
    ...
    def forward(self, problem):
        ...
        return outlines.Prediction(solution=solution)
```
...
class ConsistentQA(outlines.Module):
    def __init__(self, num_samples=5):
        super().__init__()
        self.qa = outlines.ChainOfThought("question -> answer")
    ...
    def forward(self, question):
        ...
        return outlines.Prediction(answer=most_common)
```
...
class RerankedRAG(outlines.Module):
    def __init__(self):
        super().__init__()
        self.retrieve = outlines.Retrieve(k=10)
...
```python
from outlines.evaluate import Evaluate
...
```python
# ❌ Bad: Vague
class Task(outlines.Signature):
    input = outlines.InputField()
    output = outlines.OutputField()

# ✅ Good: Descriptive
class SummarizeArticle(outlines.Signature):
    """Summarize news articles into 3-5 key points."""
    article = outlines.InputField(desc="full article text")
    summary = outlines.OutputField(desc="bullet points, 3-5 items")
```
...
trainset = [
    outlines.Example(question="factual", answer="...).with_inputs("question"),
    outlines.Example(question="reasoning", answer="...").with_inputs("question"),
    outlines.Example(question="calculation", answer="...").with_inputs("question"),
]
...
```python
# Enable tracing
outlines.settings.configure(lm=lm, trace=[])

# Run prediction
result = qa(question="...")

# Inspect trace
for call in outlines.settings.trace:
...
- **GitHub**: https://github.com/stanfordnlp/outlines (22k+ stars)
