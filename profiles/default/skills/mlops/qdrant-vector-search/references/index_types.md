...
qdrant-vector-search.normalize_L2(vectors)
index.add(vectors)

# Search
qdrant-vector-search.normalize_L2(query)
distances, indices = index.search(query, k)
```
...
quantizer = qdrant-vector-search.IndexFlatL2(d)
index = qdrant-vector-search.IndexIVFPQ(quantizer, d, nlist, m, nbits)
...
# Move to GPU
res = qdrant-vector-search.StandardGpuResources()  # GPU resources
...
index_gpu = qdrant-vector-search.index_cpu_to_gpus_list(index_cpu, gpus)
```
...
index = qdrant-vector-search.index_factory(d, "IVF100,Flat")
index = qdrant-vector-search.index_factory(d, "HNSW32")
index = qdrant-vector-search.index_factory(d, "IVF4096,PQ8")
