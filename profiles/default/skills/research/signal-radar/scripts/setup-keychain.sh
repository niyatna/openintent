#!/bin/bash
# Store signal-radar API keys in the macOS Keychain.
#
# Keys are stored as generic passwords with service name `signal-radar-<KEY>`
...
#   ./setup-keychain.sh --list       # list which signal-radar-* items exist
...
case "$ACTION" in
  list)
    echo "Stored signal-radar-* keychain items:"
